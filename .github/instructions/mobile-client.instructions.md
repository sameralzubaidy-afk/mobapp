...
- BP-91 Mobile UI changes need an in-session on-device attempt — otherwise the Session Handoff must enumerate each owed device leg concretely (screen → action → expected observation), never a generic "device verification pending"; a UI change is never "verified" from typecheck/lint/unit tests alone, budget the device pass BEFORE the code work, and re-read the AX tree/element list instead of trusting a screenshot taken immediately after a tap; and assert against a FRESHLY-FETCHED screen state, never a screen retained across a deep link (FIX-Task-25, 2026-09-13; fresh-state precondition added FIX-Task-56, 2026-09-18).
...
7. **Assert against a FRESHLY-FETCHED screen state — a retained screen makes a correct server state read as a REGRESSION (FIX-Task-56, 2026-09-18).** A deep link into an **already-mounted** route re-focuses the retained screen instead of remounting it, so the screen's in-memory data (a fetched list, a `primaryMethodId`, a derived flag) still reflects the state from BEFORE your change. In the worked case the DB already said `primary=true, verified=true, stripe_onboarding_complete=true`, yet the guard still fired with the *unverified* variant because the screen had been mounted earlier — an assertion that reads as "the fix did not work" when the fix is live and correct. **Stale screen data therefore biases toward FALSE NEGATIVES, not "no data".** Before asserting anything about server-derived state on a device: force a genuine fresh fetch (**cold relaunch**, or an explicit pull-to-refresh where the screen supports it) and confirm the screen re-mounted (a fresh-`fetch` marker, or an element that can only be present with the new data, such as the `primary-chip`). Then arbitrate against the source of truth — a read-back (`qa:payout-fixture -- status`, a DB read, `qa:stripe-inspect`) — and let the DB decide which side is stale: if the DB is correct and the screen disagrees, the screen is stale, not the app broken. Record the staleness as proof-of-fetch friction, never as an app defect (same arbitration instinct as the QA playbook's DB-over-UI rule and R96's mounted-route behaviour).

*Evidence / origin: FIX-Task-25 (2026-09-13) — the handoff for that round carried an explicit "On-device legs still owed (a)–(e)" list precisely because the device pass was deferred to the end of the session; the reviewer's own suggested rule was "a mobile screen-behaviour change must be accompanied by an on-device verification attempt in the same session, or the handoff must say exactly which device legs are owed and why". Rule 7 added by FIX-Task-56 (2026-09-18), where the first device attempt on the auto-promoted payout method returned a false negative for exactly this reason. Applied via `.github/prompts/apply-handoff-rule-suggestion.prompt.md`.*
