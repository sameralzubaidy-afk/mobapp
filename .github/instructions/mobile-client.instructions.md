...
- BP-94 A `jest.mock()` factory must mirror the module's FULL export surface — adding a new import to the module under test requires adding that export to every factory that stubs it in the SAME pass, because an unlisted export is `undefined` at call time, its `TypeError` is swallowed by the consumer's own `try/catch`, and the test then silently asserts on the wrong (fallback/error) branch while staying green (FIX-Task-28, 2026-09-13).
...
1. When you add an import to a module under test, grep every factory for that module path (`jest.mock('<that module>', …)`) in the **same pass** and add the new export. A partial factory is not "a mock of the module" — it is a module whose unlisted exports are `undefined`.
