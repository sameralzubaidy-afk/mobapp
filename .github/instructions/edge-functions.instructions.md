...
- BP-41 Edge Function deploys — REQUIRED path is the official CLI (`supabase functions deploy <name> --project-ref <ref>`), which resolves `../_shared/*` from the filesystem (structural fix, no manual file enumeration). Reconcile `verify_jwt` against `config.toml` BEFORE deploying (add an unlisted function that is deployed with `verify_jwt = false` to `config.toml` first — see `paypal-webhook`/`release-payment`, 2026-08-27). MCP bundler is last-resort fallback only; legacy `functions/_shared/` prefix / inline-and-keep-in-sync workaround retired for new work.
...
Rules:
1. **CLI is the REQUIRED deploy path for any function that imports `../_shared/*`** (or any local relative module). Run from the repo root:
   ```bash
   supabase functions deploy <name> --project-ref <ref> --yes
   ```
   The CLI uploads `index.ts` plus every resolved `_shared/` dependency as assets (see "Uploading asset" lines) and the platform bundles them — the `../_shared/*` parent-dir resolution failure is structurally removed. A "WARNING: Docker is not running" line is benign (the CLI falls back to server-side bundling). Do NOT use `--use-api` if local/asset upload succeeds — the CLI asset path is the tested one.
