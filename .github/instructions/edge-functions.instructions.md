...
- BP-97 A deployed Edge Function that has never processed ONE real delivery is UNVERIFIED — "source parity with a working sibling" proves the code shipped, not that the path can run (the `stripe-webhook` endpoint rejected EVERY Stripe event for weeks: synchronous `constructEvent` → `SubtleCryptoProvider cannot be used in a synchronous context`; found within minutes of building the signed-replay helper). Ask "has this handler ever completed one real delivery?", build the signed-delivery mechanism early, read the rejection MESSAGE not just the status, and use `await constructEventAsync` on Deno (FIX-Task-52, 2026-09-17).
...
## BP-97: A Deployed Edge Function That Has Never Processed One Real Delivery Is UNVERIFIED — "Source Parity With a Working Sibling" Is Not Evidence the Path Can Run

**Problem:** `stripe-webhook` was deployed since 2026-08-27 (v49), its source matched the deployed body, and its sibling `stripe-webhook-subscriptions` verified signatures correctly with `await constructEventAsync(...)` — so it was repeatedly cited (in docs AND in QA verdicts) as *"source + deployed-function parity verified"*. It had nonetheless **never processed a single delivery**. It used the **synchronous** `stripe.webhooks.constructEvent(body, sig, endpointSecret)`, which on the Deno / `esm.sh` Stripe v14 runtime throws `SubtleCryptoProvider cannot be used in a synchronous context. Use `await constructEventAsync(...)` instead` — **before any handler runs**. The endpoint therefore answered `400` to *every* Stripe event: `payout.created|updated|paid|failed` (seller payout status reconciliation), `charge.captured` (tax-collected safety net), `charge.refunded`, `payment_intent.payment_failed`, `account.updated` (Stripe Connect status sync), and `charge.dispute.*`.

Nothing surfaced it for weeks, and that silence was itself the tell: **no tool existed that could send a correctly-SIGNED event**, so every payout/webhook verdict quietly rested on the DB-trigger path instead. The recorded observation *"the `processing` → `completed` transition came from the DEV-TASK-124 dispatch trigger, not a provider webhook"* was literally true and **could never have been otherwise** — a webhook could not have succeeded. Found within minutes of building `scripts/qa/stripe-webhook-replay.mjs` and POSTing the first real signed event (FIX-Task-52 item 7b, 2026-09-17).

**Rules:**

1. **Never accept "source parity" / "it matches the sibling function" / "the deployed body is up to date" as verification that an endpoint WORKS.** Those prove the code was shipped. "This handler has successfully processed at least one real delivery" is a **different claim requiring different evidence**.
2. **Before reporting a webhook / provider callback / cron handler as working, ask: has it ever completed one real delivery?** If the answer is unknown or no, say so explicitly — *"deployed; never exercised — no signed delivery has ever reached it"* — instead of implying coverage.
3. **Build the delivery mechanism early — it is the cheapest test available.** For a signed/authenticated endpoint, a replay helper that mints a correctly-signed request is usually ~100 lines and converts a permanently-unverifiable leg into a one-command check. This repo ships `npm run qa:stripe-webhook-replay -- --type payout.paid --payout <po_...>` (signs as `t=<unix>,v1=<HMAC-SHA256(secret, "<t>.<body>")>`); prefer running it over reasoning about the handler.
4. **Read the rejection MESSAGE, not just the status code.** A `400` is not automatically the caller's fault — the body distinguishes "the endpoint is broken" (`SubtleCryptoProvider cannot be used in a synchronous context`) from "my secret/params are wrong" (`No signatures found matching the expected signature`). The exact string is the evidence; the error text changing IS the proof a fix landed.
5. **Use the ASYNC Stripe verification API on Deno:** `await stripe.webhooks.constructEventAsync(body, sig, endpointSecret)`. WebCrypto (`SubtleCryptoProvider`) cannot be used from a synchronous context, so the sync `constructEvent(...)` throws on EVERY call. Check sibling webhooks for the correct form rather than assuming parity means correctness.
6. **A gateway probe is the cheap way to prove both "the request reaches the handler" and "`verify_jwt` survived the deploy":** `POST` with no signature/auth and read the body — the function's OWN error (`400 Webhook Secret missing`, `401 {"success":false,"error":"Missing authorization header"}`) means it ran; a gateway `401` means it did not (see BP-19).

**See also:** BP-66 (a "Deployed" message is not evidence the deploy happened), BP-41 rule 5 (mandatory post-deploy real invocation), BP-87 (DB-trigger/cron-invoked EF auth), BP-19 (`verify_jwt = false` for cron-invoked EFs), and the Hardening Protocol clause requiring real invocation of EVERY code branch after a `CREATE OR REPLACE FUNCTION` — **BP-97 is that same "an unexercised path is not verified" discipline, for Edge Functions.**
