...
### 5.31 Maintain and consult a known-undrivable-native-modal list (Phase 13.38)

Three separate runs have now hit the same wall: a native full-screen modal (`CategorySelectModal`, the Sell/bulk-upload sheet, and now `expo-image-picker`'s crop/confirm editor) that is not in the accessibility tree and rejects synthesized taps and swipes entirely. Maintain a standing list in memory of screens/flows known to trigger this pattern (any `presentationStyle="fullScreen"` native picker, system camera/photo UI, OS-level crop/confirm dialogs). Before executing a case that touches one of these, check the list first: if the target screen is already known to be undrivable, do not spend bounded-attempt cycles rediscovering that fact — go straight to recommending/using a `__DEV__` fixture bypass if one exists, or if none exists, note the gap and pivot immediately (still bounded to 1–2 confirmation attempts to verify the specific instance behaves the same way, not the full 5+ attempts used when discovering the pattern for the first time). Known instances so far: `CategorySelectModal` on ItemCreate (Phase 22/23), the Sell/bulk-upload sheet, and `expo-image-picker`'s `allowsEditing` crop/confirm window (Phase 13.38 Group H). Do not re-derive these per-run — consult the list first.
