...
Full bug-prevention rule text below: BP-1, BP-2, BP-3, BP-4, BP-5, BP-6, BP-9, BP-10, BP-11, BP-12, BP-13, BP-14, BP-16, BP-21, BP-22, BP-30, BP-31, BP-37, BP-38, BP-44, BP-45, BP-46, BP-47, BP-48, BP-73, BP-74, BP-75, BP-76, BP-78, BP-79, BP-80, BP-81, BP-84, BP-90, BP-96, BP-101. (BP-19 cron `verify_jwt` lives in `edge-functions.instructions.md`; BP-20 notification-trigger check and BP-32 notification verification gate live there too.) See the Bug Prevention Rule Index in `Kids P2P App Builder.agent.md` for the one-line summary of all rules.
...
- BP-48 Admin config writes — settings MUST go through the shared `upsert_admin_config_setting(p_admin_id)` RPC, never direct `admin_config` table writes (records the editor + lands in the shared audit trail); and a value-only write must not move the row's sibling columns (`category`/`data_type`/`is_active`) — helpers that carry defaults (e.g. `qa:admin-config-set`) clobber them, so pass them explicitly on the set AND the revert.
...
- BP-76 Enum-like status/reason literals — DB writers must emit the canonical snake value (`'offer_expired'`) that triggers AND the client match exactly; never store a display string (`'Offer expired'`) in a machine-compared column (silently breaks surfacing/counters, TRD re-verify 2026-08-28). When a CHECK constraint legally admits TWO spellings of one state (live: `subscriptions.status` = `'grace'` AND `'grace_period'`), every server gate + client allow-list must accept both AND the data must be normalized — and count the distribution (`SELECT status, count(*) … GROUP BY status`) before trusting any status-driven rule, because one legacy-spelled outlier row is a fixture defect that fakes a product-level failure (FIX-Task-51, 2026-09-17).
...
- BP-79 Default-privilege hardening is ineffective for NEW functions on Supabase PG 17.6 — `ALTER DEFAULT PRIVILEGES ... REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC` cannot stop them being client-executable (built-in PUBLIC baseline always applies AND the default-ACL row directly grants anon/authenticated); enforce fail-closed defaults via the `dt61_guard_revoke_fn_public` EVENT TRIGGER (auto-REVOKE PUBLIC, anon, authenticated on every new public-schema function/procedure) + explicit-grant discipline (DT-61, 2026-08-30). **The guard fires on `CREATE OR REPLACE FUNCTION` too (same `command_tag`), so REPLACING an existing function strips its PUBLIC/anon/authenticated grants while untouched siblings keep theirs — re-assert `GRANT EXECUTE` in the same migration and verify live per consumer role (`has_function_privilege` + before/after `aclexplode(pg_proc.proacl)` diff); measured blast radius: the client-called `get_subscription_summary` (FIX-Task-51, 2026-09-17).**
...
- BP-96 Re-measure the WHOLE migration chain after any fix that flips a previously-failing file to passing (a newly-succeeding early file creates state a later file assumed absent), watch the pass-1 applied count as the canary, read a deferred-replay probe's per-file error as its LAST error rather than the root cause, enumerate a chain's gap by diffing the live schema against the replayed schema — never by reading the failure list — and **re-run the probe after EVERY migration file you add or edit, not only after a repair** — FIX-Task-40, 2026-09-16 / FIX-Task-57, 2026-09-18.
- BP-98 A fixture/provisioning script must only assert a state the PRODUCTION WRITERS can create — enumerate every writer of the columns it fakes before trusting it; a fixture that hand-sets an impossible combination does not merely green-light a branch, it can SKIP the very gate it was built to test (a fixture setting `is_primary=true, is_verified=false` bypassed the withdraw guard, so the case sat "blocked" on a state only the fixture could produce — FIX-Task-55/56, 2026-09-18).
- BP-99 Migration filenames are ORDER KEYS, and the CLI only applies files matching `<digits>_<name>.sql` — anything else is silently skipped (never even logged as `Applying migration …`), a legacy-numbered name sorts BEFORE every 14-digit name, and a stale legacy-named duplicate replaying first hides the real state of the chain; never renumber by hand and never infer a file's age from its prefix (FIX-Task-57, 2026-09-18).
- BP-100 A schema-diff residual is EVIDENCE, not a to-do list — classify every difference by DIRECTION before backfilling (staging-ahead → backfill it; chain-ahead → the repo carries newer behaviour, don't reconcile it back; staging-looser/unsafe → a STAGING finding for an owner decision, never a silent chain change; dead/cruft → don't propagate), grep the object's own history for a deliberate `DROP` before re-adding it (`admin_config_authenticated_read` had been removed on purpose by the prod security lockdown — restoring it would have exposed the whole admin config to every signed-in user), and never drive the count to zero: the only regression signal is a NEW finding (`comm -13` of the key sets must be empty) — 102 of 135 survivors were benign-by-design, 74 of them a `search_path` delta where the REPLAY is the stricter side; also name the gate's blind spots (no `prosrc` body, no `proacl` grants — 21 client-facing RPCs were uncallable on a rebuilt DB while the gate stayed silent); and **ATTRIBUTE a NEW finding before reporting it** — temporarily remove your own artifact, re-measure, and treat key-set-identical output as proof the delta is not yours (another session's uncommitted migration moved the gate 135 → 136 in FIX-Task-61, 2026-09-18) (FIX-Task-60, 2026-09-18).
- BP-101 Staging schema changes are FILE-FIRST — write the migration file, then apply it; never apply first and backfill a file "later" (later does not happen), and never leave an applied change sitting uncommitted or existing only in a working tree (the drift this task was created to end). Reserve the migration NUMBER before you write the file (`node scripts/migrations/reserve-number.mjs --next`, or `--block` when a parallel session is live), and let the gate — not memory — decide: `npm run migration-gate` locally, `.githooks/pre-commit` as a preview, and the required `Migrations gate` CI job as the enforcement (FIX-Task-63, 2026-09-18).
...
- A write that only intends to change `value` MUST NOT move the row's sibling columns (`category`, `data_type`, `is_active`). Callers that carry their own defaults clobber them silently — `qa:admin-config-set` defaults to `--category feature_flags --data-type string`, so even a correctly-restored `value` leaves `min_listing_price` re-typed as `string` where its seed migration says `number` (`fees`). Pass the row's real `--category`/`--data-type` on every set AND on the revert (FIX-Task-29, 2026-09-13).
...
Detection checklist: grep for direct `admin_config` INSERT/UPDATE outside `upsert_admin_config_setting`/`secure_upsert_admin_config`; confirm each settings page passes `p_admin_id`/`user_id`; confirm every settings-edit surface writes `admin_audit_log`; confirm audit target tables exist. After any config write OR revert, read the WHOLE row back (`SELECT key, value, category, data_type, is_active, updated_by`) and compare `category`/`data_type` against the seed migration that defines them — a `value`-matches read-back is not proof of zero residue. Cross-ref BP-11 (read both `admin_config` and `sp_config`; don't trust `is_active` alone) and HP-5 (atomic multi-table mutations via a single RPC).
...
- **A CHECK constraint that admits two spellings of one state makes every gate a two-spelling question (added 2026-09-17).** When the column's CHECK allows a legacy alias next to the canonical value, EVERY server gate and EVERY client allow-list must accept both spellings — or the data must be normalized so only one can exist. Live example: `subscriptions.status` legally allows both `'grace'` and `'grace_period'`; the client accepted both (`src/services/subscription.ts` `canSpendSp`) while the server gates (`can_user_spend_sp`, `fn_get_sp_entitlement`, `get_subscription_summary`) listed only `'grace_period'` — so the one `'grace'`-spelled row (a QA persona) was offered SP by the app and refused by the server (HTTP 403 `SP_NOT_ENTITLED`), i.e. a status-driven rule silently disagreed with the data it was reading.
- **Count the DATA before trusting a status-driven rule (added 2026-09-17).** Run `SELECT status, count(*) FROM <table> GROUP BY status ORDER BY 2 DESC` (and the same for any reason/state column a gate branches on) BEFORE believing an "X is blocked/not allowed" claim or a status-derived counter. A lone outlier row is a fixture/data defect that can FAKE a product-level failure — measured live: `'grace'` = 1 row vs `'grace_period'` = 446, and that single row was exactly the QA grace persona under test, which cost a whole money-path investigation round. Fix BOTH faces: normalize the outlier to the canonical spelling AND widen the gates, so a future legacy write (or a replayed old migration) cannot reintroduce the contradiction.

Detection checklist: an expired/declined offer missing from the app's "Your Offers", or a counter resetting unexpectedly on expiry, with no error → compare the client/trigger literal against the live DB value in `trades.cancellation_reason`; a `'offer_expired'` vs `'Offer expired'` difference is the bug. **Status-driven gate refusing a state the user is demonstrably in** → `SELECT status, count(*) … GROUP BY status` + diff the gate's literal list against the DATA before believing it (a CHECK that allows a legacy spelling means the gate is wrong, not the user). Cross-ref BP-3 (silent mis-filtering), BP-16 (stale trigger comments), BP-31 (verify both trigger AND RPC layers), BP-78 (identity/allow-list gates on money RPCs).
...
- **PostgREST maps `42501 insufficient_privilege` to HTTP 401, not 403** — in negative-check grant verification, treat a 401 from a revoked caller as the EXPECTED rejection signal (don't flag it as an unexpected mismatch). A 401 from a revoked caller and a 401 from a stale service-role key are indistinguishable, so before trusting a lockdown result, confirm which env file the running server/EF actually loaded (a stale key produces the identical symptom as a correctly-tightened grant — false positives/negatives otherwise; DT-45/47, 2026-08-29).

Detection checklist: any money RPC that is (a) granted to `anon`/`authenticated`, (b) PUBLIC-by-default (no `REVOKE FROM PUBLIC` anywhere), or (c) trusts a self-declared `p_user_id`/`p_admin_user_id` with no `auth.uid()` gate → Tier-0 finding. Verify the grant against LIVE `aclexplode(pg_proc.proacl)` before AND after any lockdown, and pair every destructive grant change with a stated one-line rollback plan confirmed BEFORE execution. Cross-ref BP-1 (RLS on new tables), BP-22 (secrets from config), BP-75 (live-vs-grep audits for RLS).

## BP-79: Default-Privilege Hardening for Functions Is Ineffective Here — Use the `dt61_guard_revoke_fn_public` Event Trigger + Explicit-Grant Discipline
Problem (Dev Task 61, 2026-08-30): the forward-looking guardrail "new functions must not default to client-executable" CANNOT be implemented via `ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC`. Proven live on Supabase PG 17.6 (staging `drntwgporzabmxdqykrp`): (a) a brand-new function STILL gets the built-in `{PUBLIC, owner}` baseline even with a clean default-ACL row — `pg_default_acl` can only ADD grants, never remove the built-in PUBLIC-execute for functions; and (b) this project's default-ACL row directly grants `anon`/`authenticated` EXECUTE to every new function (grantor postgres, pg_default_acl objtype 'f'), so a PUBLIC-only REVOKE leaves them executable anyway (post-apply probe: `anon_exec=true, auth_exec=true` until anon/authenticated were also revoked).

Rules (standing playbook — Dev Task 61 §5):
- **Do NOT rely on `ALTER DEFAULT PRIVILEGES ... REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC`** to fail-close new functions in this environment — it does not work (built-in PUBLIC baseline + default-ACL direct grants to anon/authenticated both survive).
- **Fail-closed default is enforced by the installed EVENT TRIGGER** `dt61_guard_revoke_fn_public` (migration `20260830000013_dev_task_61_event_trigger.sql`, APPLIED to staging 2026-08-30): on `ddl_command_end` for `CREATE FUNCTION`/`CREATE PROCEDURE` in the `public` schema, it auto-runs `REVOKE EXECUTE ON <obj> FROM PUBLIC, anon, authenticated`. `service_role` keeps its default EXECUTE (privileged backend; EFs call via the service key). Removing the guard requires owner sign-off (rollback: `DROP EVENT TRIGGER IF EXISTS dt61_guard_revoke_fn_public; DROP FUNCTION IF EXISTS public._dt61_guard_revoke_fn_public();`).
- **Every new public-schema function/procedure MUST carry an explicit `GRANT EXECUTE ON FUNCTION ... TO <anon|authenticated|service_role>` in the SAME migration that creates it** — otherwise it is executable only by its owner (postgres). This is now an enforced discipline, not optional; a missing grant manifests as 42501/HTTP 401 at runtime.
- **`CREATE OR REPLACE FUNCTION` fires the same guard — replacing an existing function STRIPS its grants (added 2026-09-17).** The event trigger's `WHEN TAG IN ('CREATE FUNCTION','CREATE PROCEDURE')` matches the replace path too (a `CREATE OR REPLACE` reports the same `command_tag`), so the auto-REVOKE runs against an ALREADY-GRANTED function and nothing re-applies the original migration's `GRANT`s. Measured live (FIX-Task-51): after `CREATE OR REPLACE` on `can_user_spend_sp`, `fn_get_sp_entitlement`, and `get_subscription_summary`, their ACLs fell from `{PUBLIC, anon, authenticated, postgres, service_role}` to `{postgres, service_role}` while untouched siblings (`get_subscription_status`, `get_user_sp_wallet_summary`, `fn_item_effective_sp_cap`, `fn_reserve_sp_on_offer`) kept the permissive baseline — that asymmetry is the guard's signature. Blast radius in this case: `get_subscription_summary` is called by the CLIENT (`AuthContext.tsx` / `services/auth.ts`) with a user JWT, so the replace would have broken session refresh for every signed-in user. **Therefore every `CREATE OR REPLACE` migration in `public` must (a) re-assert `GRANT EXECUTE` for every consumer role it had, in the same file, and (b) verify the result live** — `SELECT has_function_privilege('authenticated','public.<fn>(<args>)','EXECUTE')` per consumer role, plus a before/after `aclexplode(pg_proc.proacl)` diff (the only reliable detector; a migration grep shows the GRANTs you wrote, not the ACL that survived). Pair with BP-90 (a `CREATE OR REPLACE` success is not evidence — invoke the object) and BP-81 (verify the change landed by live invocation).
- **`REVOKE EXECUTE ... FROM anon` is a SILENT NO-OP while the function still carries the built-in PUBLIC grant (added 2026-09-18, FIX-Task-59).** `anon`/`authenticated` INHERIT EXECUTE *through* PUBLIC, and revoking from a named role cannot remove an inherited privilege — so a lockdown that revokes only `anon` reads as closed while the entry point stays wide open. Measured live (staging, FIX-Task-59): after `REVOKE EXECUTE ... FROM anon` on `verify_phone_code`, `is_phone_verified` and `check_phone_verification_status`, `has_function_privilege('anon', <fn>, 'EXECUTE')` was still `TRUE` for all three and each ACL still read `=X/postgres | postgres=X | authenticated=X | service_role=X` — the `=X/postgres` entry IS the PUBLIC grant. The same statement DID work on `verify_user_phone`, whose PUBLIC grant the guard had already stripped, leaving an explicit `anon=X` to remove; that asymmetry is the tell. **Therefore revoke from `PUBLIC, anon` (never `anon` alone) and then re-assert `GRANT EXECUTE` for every role that must keep access** — dropping PUBLIC removes the inherited path for all non-owner roles, so those explicit grants become load-bearing. Verify with `has_function_privilege` per role, never by re-reading the statement you just ran. This is the mirror image of BP-78's GRANT-side rule (`GRANT` without `REVOKE FROM PUBLIC` leaves PUBLIC executable).
- **Do NOT use the same migration to "tidy" a pre-existing over-broad grant** (e.g. closing a legacy `PUBLIC` EXECUTE) as a side effect of a vocabulary/body fix — that is a platform-wide security decision with its own blast-radius analysis (BP-78's caller analysis + rollback plan), not a freebie; restoring the previous posture and filing the hardening separately keeps the change reviewable and reversible.
- The grant-discipline audit that justified the trigger: 429 public functions, 370 PUBLIC-executable, but **0 rely on built-in PUBLIC without an explicit anon/authenticated grant** — every client function is already explicitly granted, so the trigger is safe for existing functions it never touches (**no retroactive effect on untouched functions** — but see the `CREATE OR REPLACE` bullet above: REPLACING one does strip its grants) and consistent with existing practice.

Detection checklist: any NEW migration that creates a public-schema function/procedure WITHOUT an explicit `GRANT EXECUTE` is a Tier-0 finding (it will be owner-only at runtime). **Any `CREATE OR REPLACE` of an existing public-schema function is the same Tier-0 hazard one step later** — the guard revokes what the original migration granted, so the replace must re-assert the grants and the result must be verified live per consumer role (`has_function_privilege` + `aclexplode` diff; see the `CREATE OR REPLACE` bullet above). **And any lockdown that revokes from a NAMED role (`anon`/`authenticated`) without also revoking `PUBLIC` is a silent no-op — re-verify with `has_function_privilege` rather than trusting the statement** (see the PUBLIC-revoke bullet above; FIX-Task-59, 2026-09-18). Any attempt to "fix" the new-function default with `ALTER DEFAULT PRIVILEGES ... REVOKE ... FROM PUBLIC` should be rejected as ineffective (see above) — use the event trigger + explicit grants. Cross-ref BP-78 (existing-function grants — per-function REVOKE works there), BP-75 (live-vs-grep audits), BP-81 (prove the change by invoking it), BP-90 (DDL success is not evidence).
...
See also: BP-81 (MCP-applied migrations aren't in `list_migrations`), BP-47 (verify the attached/deployed body), BP-74 (assert on the linkage key), BP-98 (a fixture's asserted STATE must be reachable by the production writers — the design companion to this rule's read-back requirement).
...
## BP-96: A Fix That Makes a Previously-Failing Migration Succeed Can Break a LATER File — Re-Measure the Chain, and Read a Deferred-Replay Probe's Error as the LAST Error, Not the Root Cause

Problem (FIX-Task-40, 2026-09-16): repairing a migration replay has two failure modes that both look like progress.

**(a) The newly-passing file breaks a later one.** `084_add_pg_cron_send_message_emails.sql` contained `'*/30 * * * *'` inside a `/* … */` block comment — the `*/` closed the comment early, so the file died with `syntax error at or near "30"`. Fixing it made the file SUCCEED, and `084` sorts **earlier** than the FIX-38 base-schema repair (`20241213000003`). `084`'s first statement is `CREATE SCHEMA IF NOT EXISTS cron` (a fallback for environments without pg_cron), and pg_cron's install script runs a bare `CREATE SCHEMA cron` — so the repair's `CREATE EXTENSION IF NOT EXISTS pg_cron` then aborted the ENTIRE repair with `schema "cron" already exists`, silently removing `public.trades` and cascading into ~77 files. Measured: **448/523 applied → 347/527**, i.e. the "fix" was net-negative until the interaction was found. Neither `CREATE EXTENSION IF NOT EXISTS pg_cron` nor `… WITH SCHEMA cron` works while an empty `cron` schema exists (both verified live).

**(b) The probe's error is the LAST error.** `scripts/migrations/replay-probe.mjs` applies every file, DEFERS failures and retries, but overwrites each file's recorded error every pass — so the error it reports is from the FINAL attempt, often a *deferral artifact* rather than the root cause (a function whose signature appears to "change" only because a different file created a different version first). The FIX-38 blocker list ("3 creator-less tables + a long tail") came from that artifact-level view; diffing the live catalogue against the replayed schema showed the real gap was **30 tables, 82 functions and 124 RLS policies** — an order of magnitude larger, and the reason renumbering alone could never have made `db reset` pass.

Rules:

1. **After any fix that turns a previously-failing migration into a passing one, re-run the WHOLE chain and compare the TOTAL — not just the file you repaired.** A newly-succeeding early file creates state (schemas, extensions, tables, enum labels) that a later file may have implicitly assumed absent.
2. **Watch the pass-1 "applied" count as the canary.** A large drop in files applied on pass 1 means a newly-succeeding file broke others — visible immediately, long before the run finishes and long before you would otherwise notice.
3. **Read a deferred-replay probe's per-file error as "last error", not "root cause".** To get the real cause, re-run that one file alone against the settled database (`psql -v ON_ERROR_STOP=1 -f <file>`) and use THAT error; never file a bug from the probe's map without isolating it.
4. **Enumerate a chain's gap by DIFFING the live schema against the replayed schema — never by reading the failure list.** Fingerprint both sides (columns, constraints, indexes, functions, enums, policies, RLS flags, views) and diff. A failure list is a symptom set; the fingerprint is the spec, and it is what tells you whether a "creator-less table" is one of three or one of thirty. Two traps: a creator search must be **case-insensitive and match `CREATE [OR REPLACE] FUNCTION`** (a `CREATE FUNCTION <name>` pattern misses every `CREATE OR REPLACE` and yields a false "no creator anywhere" — see 9.1d), and a pipe-delimited fingerprint dump must **strip newlines** (`replace(qual, E'\n', ' ')`) because `pg_policies.qual` and function bodies are multi-line — otherwise the dump truncates them and the sort scrambles the keyspace into phantom differences.
5. **Prefer creating an object CORRECTLY at its creator over repairing it later.** FIX-38 retyped the node-id columns to `uuid` in a later repair migration, which worked only when it ran before `20241214000003` created the `profiles_with_auth` view — and the two files are adjacent in filename order, so any deferral made it abort with *"cannot alter type of a column used by a view or rule"* and fail ~130 downstream files. Declaring `nodes.id` / `profiles.node_id` / `zip_codes.node_id` as `uuid` **in the migration that creates them** made the later retype an unnecessary no-op and removed the ordering coincidence entirely. A later repair is always order-fragile; fix the creator when you can.
6. **Compare the WHOLE per-pass ladder, not just pass 1** (refines rule 2 — FIX-Task-40 phase 3, 2026-09-16, proved the single-number canary insufficient). Hoisting one enum type made `unresolved` go **1 → 3** while **pass 1 stayed flat at 389**; what caught it was **pass 2 dropping 135 → 133**. Record pass 1/2/3/… after every change and compare every rung — a regression hides behind an unchanged headline.
7. **A statement referencing a signature the file does not itself create is order-fragile.** Four files did `COMMENT ON FUNCTION <fn>(date, date, uuid, text)` while creating the **five**-argument version — perfectly fine in strict filename order, fatal the moment another file DROPs the 4-arg overload first. Same class: any `GRANT` / `COMMENT` / `DROP … IF EXISTS` aimed at a signature only a *different* file creates. Grep the signature and fix the **whole class**, not the single instance the probe happened to report.
8. **When two files both define the same object, the earlier one must apply first — and the fingerprint cannot see the failure.** `20260724000001` must precede `20260724000002`, or it re-creates `get_tax_summary_for_period` with its **older body**; the fingerprint records a function's identity / returns / security / config, **not its body**, so the gate stays green while behaviour silently regresses. If deferral keeps putting the wrong file first, fix the CAUSE of the deferral (hoist the type/dependency so the pair resolves in pass 1) — do **not** add a `DROP` prologue, which just guarantees the older body wins in every order.
9. **The probe is NOT a substitute for `supabase db reset` — the CLI's migration session can have different privileges.** Statements that apply cleanly through `psql` failed under the CLI with 42501 (`permission denied for function pg_read_file`, `must be owner of table objects`) because that session runs as `postgres` with **`rolsuper = false`** and is not a member of the role owning `storage.objects`. Before "fixing" a migration to make the reset pass, establish whether the failure is CONTENT or EXECUTION CONTEXT — and never fail-soft a security-relevant statement (`CREATE POLICY` on `storage.objects`) just to get green. That needs an owner decision and is a Blocked-Tier item, never a quiet carry-forward.
10. **Re-run the probe after EVERY migration file you ADD or EDIT — not only after a repair** (FIX-Task-57, 2026-09-18). The FIX-Task-51 file landed **one day after** the chain was last proven green and immediately re-broke it: `531/531` → `530/531`, `unresolved: 1`. The shape to expect is a **newly-added** file — or an existing file whose renumbered key puts it after the generation that owns an object — re-declaring that object with a narrower shape. Measured instance: `20260916000064_fix_trade_sp_credit_integrity.sql` is a RENUMBERED LEGACY file (its own header still reads `090_fix_trade_sp_credit_integrity.sql`) numbered **after** the R6 file that defines the current 5-column `get_subscription_summary` row type; it re-installed the **pre-R6 4-column** row type, so the newer file's `CREATE OR REPLACE` was refused with `cannot change return type of existing function | DETAIL: Row type defined by OUT parameters is different` — the rebuild then **silently kept the older shape**. The author's "signature is UNCHANGED, no DROP needed" was verified against **live staging** and was false **on a fresh replay**: a claim checked against the live database says nothing about the replay. Fix in the file numbered LAST (a `DROP FUNCTION IF EXISTS` prologue — BP-12 — plus re-asserted grants, BP-79), then re-run the probe immediately. The check costs minutes and is the only thing that catches this class.

See also: BP-9 (migration dependency order), BP-47 (the latest definition is authoritative), BP-80/BP-81 (state what actually ran and verify the object, not the log), and agent rule 9.1e (reproduce a dispatched fix's PREMISE before implementing its remedy — this rule is the mechanical companion: 9.1e says measure the FULL failure set, BP-96 says measure it AGAIN after every fix, and by fingerprint rather than by failure list).

Detection checklist: after any migration-chain repair, compare **every** pass's applied count against the previous run (not just pass 1 — see rule 6) before anything else; a drop on ANY pass means a newly-succeeding file broke others. Then confirm `unresolved` strictly decreased; a repair that leaves the total unchanged or worse is a regression, not partial progress. Before reporting any remaining blocker, re-run that file in isolation to confirm the probe's error is its real cause. Before believing an "object has no creator" claim, re-grep case-insensitively for `create … function` AND `create or replace function`. Before trusting a harness's findings, dry-run its matcher on a known pair — this round's own checker produced four phantom "NO CREATOR" hits from newline-split policy definitions and normalised `uuid_generate_v4()`/`gen_random_uuid()` on only one side. And before declaring a chain renumbered, confirm the probe reports **`pass 1: applied <N>, deferred 0`** — a single-pass, zero-deferral replay is the property `db reset` actually requires. And after adding or editing ANY migration file (not only after a repair), re-run the probe and require the same `pass 1: applied <N>, deferred 0` with `unresolved: 0` — a file added after the chain was last green is the likeliest thing to have broken it, and a file whose NAME the CLI silently skips will not even appear in the run (BP-99 rule 2).

## BP-98: A Fixture Must Only Assert a State the PRODUCTION WRITERS Can Create — an Impossible Fixture Can Skip the Very Gate It Was Built to Test

Problem (FIX-Task-55 item 2 / FIX-Task-56, 2026-09-18): a QA fixture scenario set `seller_payout_methods` to `is_primary = true, is_verified = false` to drive the withdraw guard's "unverified method" branch. That combination **cannot exist in production**: the only writer of `is_primary = true` is `set_primary_payout_method()`, which refuses any method that is not already `is_verified = true`. Because the fixture made `is_primary` true, the app's `!primaryMethodId` gate was **never entered at all** — so the guard's unverified branch had never fired on a device, and the case was carried for weeks as "BLOCKED (fixture gap)" while being perfectly reachable in production. The pairing was doubly misleading: the blocked-ness looked like an *app* limitation, and the fixture looked *thorough* because it exercised the named status field.

Rules:

1. **Before trusting a fixture's state, enumerate the writers of every column it fakes.** Grep for the writers (`INSERT`/`UPDATE` sites, EF inserts, RPCs, triggers, service-layer helpers) and ask of each: *can this code produce the combination the fixture sets?* A fixture is a model of production; if no writer can build its row, the fixture is testing a state that exists only inside the fixture.
2. **A fixture that cannot occur in production does not merely green-light a branch — it can BYPASS it.** The dangerous variant is not "the fixture made a dead branch pass" (BP-88's mock-shape face) but "the fixture made the GATE evaluate false", so the code path under test never runs and the test reports a different outcome entirely. Check whether the fixture's row satisfies or defeats the guard that decides whether your branch is reached.
3. **When a dispatched case is "blocked on a fixture gap", settle the REACHABILITY question from source before accepting the blocked verdict** — read the writer that would have to create the state. "Blocked" is a claim (agent rule 9.1e second face); if the state IS reachable, the deliverable is a corrected fixture, not a retired case.
4. **Add the correcting scenario rather than editing the wrong one.** Where an existing scenario encodes an impossible state, keep it only if something still depends on it and add the production-reachable sibling (`single-verified-nonprimary` alongside `single-unverified-nonprimary`); a self-correcting scenario is the strongest form — the newly-added DB rule promotes it on insert, so the fixture's own read-back proves the rule fired.
5. **State the fixture's boundary in the scenario itself.** If a scenario can drive only part of a path (a synthetic `acct_...` reaches the guard but cannot mint a real hosted-onboarding link), say so in the scenario's comment so the next round does not report the unreachable remainder as a defect.
6. **A fixture's asserted state must also be reachable by the MONEY paths, not just the screen you are testing.** The same `is_primary` flag read by the withdraw guard is read by `rpc_create_payout_on_trade_complete`, `initiate-payout` and `request_seller_payout`; fixing only the UI-facing reader would have shipped a screen that says "you're all set" while earnings kept stranding as `requires_action`.

See also: BP-88 rule 6 (the MOCK-side twin — a mocked SDK error whose shape the real SDK never emits keeps an unreachable branch green), BP-80 (provisioning phases + the read-back must print primary keys), BP-72 (verify a side effect directly, not via the UI), agent rule 9.1e (a REACHABILITY precondition is a claim — read the owning source before accepting it), and agent rule 9.1g (trace the READERS of a field before choosing which layer fixes it — rule 6 above is that discipline applied to fixtures).

Detection checklist: any case carried as "BLOCKED (fixture gap)" or "unreachable until X" whose state was **never** produced by a real code path → suspect an impossible fixture first. Grep every writer of the columns the fixture sets, confirm at least one can produce the combination, and if none can, rebuild the scenario from the writer's actual behaviour and re-drive the case.

## BP-99: A Migration Filename Is an ORDER KEY — and the CLI Silently Skips Any File That Does Not Match `<digits>_<name>.sql`

Problem (FIX-Task-57, 2026-09-18): two filename traps around `supabase/migrations/`, and both fail **quietly** — neither raises an error you can see.

**(a) The CLI only applies files matching `<digits>_<name>.sql`.** A diagnostic file named `20241214000004z_ft57_tmp_diag2.sql` was placed in the directory with the correct numeric prefix and the correct sort position, and the CLI **never applied it and never mentioned it** — no `Applying migration …` line, no error, nothing. As far as the run was concerned the file did not exist. It had been written specifically to instrument the CLI's own migration session, so the missing output was first misread as "the probe printed nothing" — a *harness* bug — rather than as a filename defect. Treat a file that never appears in the run's output as **not loaded**, and check its name before its contents.

**(b) A filename is an ORDER KEY, not a date.** The FIX-Task-42 renumbering deliberately gave the legacy-numbered files (`006_…`–`319_…`) `20240101…`-style keys so that lexicographic filename order IS the dependency-valid order. Consequences: a migration's version prefix no longer indicates when it was written, and legacy files legitimately sort **before** or **after** newer files that supersede them. Getting this wrong is not cosmetic — while legacy prefixes existed they sorted ahead of *every* 14-digit name, so a single stale `077_add_auto_payout_admin_config.sql` duplicate replayed **first** and killed `db reset` with `relation "public.admin_config" does not exist`, long before any real error and thus hiding the true state of the chain. Seven such byte-identical leftovers had to be deleted after the renumbering (each `cmp`-verified against the copy it was removed against).

Rules:

1. **After adding or renaming ANY file in `supabase/migrations/`, assert the directory's shape before trusting a run:** `ls supabase/migrations | grep -E '\.sql$' | grep -vcE '^[0-9]{14}_'` must print **0**. A file that counts as non-conforming is not applied — it is silently dropped.
2. **If a file appears not to have run, check its NAME before its contents.** The absence of an `Applying migration <name>` line means the file was not eligible or not found; it does **not** mean its SQL was wrong. Do not debug SQL for a file the CLI never loaded.
3. **Never renumber migrations by hand**, and never infer a file's age or precedence from its version prefix. Order is DERIVED from what actually applies, so after any repair that could move it, re-derive it with the tool (`node scripts/migrations/renumber.mjs`, dry-run first) rather than moving files yourself.
4. **A renumbering is not durable until it is committed and re-verified.** The seven duplicate leftovers above reappeared *after* the phase-3 renumber had completed — run rule 1's one-line check before a run, not once per renumber.
5. **When two files define the same object, the earlier-applying one wins** — so a legacy file that re-declares an object's SHAPE (row type, not just its body) either makes the newer file fail outright or silently installs the older shape. Prefer fixing the ORDER; otherwise have the last-numbered file install the current shape explicitly (BP-12, BP-96 rule 10).

See also: BP-96 (re-measure the whole chain; re-run the probe after every file; probe errors are last-errors), BP-9 (migration dependency order), BP-12 (`DROP FUNCTION` before a row-type change), BP-47 (the latest definition is authoritative).

Detection checklist: a migration file that exists on disk but never appears in the run's `Applying migration …` output → check its filename against `<digits>_<name>.sql` first. A `db reset` failing implausibly early (`relation "X" does not exist` for a base table) → look for a stale legacy-named duplicate sorting ahead of the 14-digit chain. Before declaring a chain's order correct, confirm BOTH that the non-conforming-name count is **0** and that the probe reports `pass 1: applied <N>, deferred 0`.

## BP-100: A Schema-Diff Residual Is EVIDENCE, Not a To-Do List — Classify Every Difference by DIRECTION Before Backfilling, and Check the Object's Own History Before Re-Adding It

**Problem (FIX-Task-60, 2026-09-18):** the fidelity gate reported **119 staging-only objects** plus **114 conflicts** (same name, different definition). The tempting reading — "the chain is missing 119 things, so backfill them until the count is zero" — is wrong, and acting on it would have made the database **less** safe. The single evidence set contained all four of these at once:

* **A genuine chain defect that MUST be backfilled.** `get_user_sp_wallet_summary` was missing `reserved_points` because a renumbered legacy file applied *after* the newer definition (BP-96 rule 10); the client maps the absent field to `0`, so **reserved SP displayed as zero**. Likewise `items_status_check` had lost `needs_edits`, so a rebuilt DB rejected the status the request-edits flow writes.
* **A deliberate removal the diff cannot distinguish from a gap.** `admin_config.admin_config_authenticated_read` (SELECT, `authenticated`, `USING true`) exists on staging and not in the replay — because `20260916000158_prod_p1_stage_security_lockdown.sql` **dropped it on purpose** in the same block that REVOKEs anon/authenticated on the table. "Restoring" it would have handed every signed-in user the entire admin configuration. Same shape: `items."Items visibility based on status"`, deliberately replaced by the node-isolation hardening in `20260916000021_prod_p1_node_isolation_hardening.sql`; re-adding it would have widened item visibility back to the pre-node-isolation model.
* **A class where the REPLAY is the stricter, correct side.** 13 staging-only `*_anon_*` policies grant the unauthenticated role read/write on `profiles`, `referrals`, `subscriptions`, `user_notifications` and available `items`; `badges."Admins can update badges"` is `USING (true)` on staging (any signed-in user can update badges). These are **staging** findings — the chain must not adopt them.
* **A class that is noise.** 74 of the 114 conflicts were a `search_path` delta only: the replay pins `search_path=public, pg_temp`, staging leaves it mutable — the replay is the *safer* side.

So "staging has X, the replay does not" is not a defect statement. It is a question — *which side is authoritative?* — and the answer differs per item.

**Rules:**

1. **Classify DIRECTION before authoring any backfill.** For each difference, decide which of four directions it is, and say so in writing: **staging-ahead** (the chain is genuinely missing something real → backfill it), **chain-ahead** (the repo carries newer behaviour and staging is behind the repo head → do NOT "reconcile" it back), **staging-looser/staging-unsafe** (the replay is the stricter side → a STAGING finding, owner decision, never a silent chain change), **dead/cruft** (redundant, superseded, or unreachable on both sides → do not propagate).
2. **Grep the object's OWN HISTORY before adding it back.** For anything the diff reports as missing, search migrations for a deliberate removal first — `grep -rn "DROP POLICY IF EXISTS <name>\|<name>" supabase/migrations/` — and read the file that removed it; the removal's header usually states the reason ("Restrict admin_config to service_role only"). This is the §9.1c sibling-precedent discipline applied to *history*: read what the codebase already decided before reversing it.
3. **"Close the count to zero" is never the goal.** The gate's job is to expose differences; deciding each one is the work. Report the residual **classified with a reason per item**, and treat a *newly introduced* finding as the only true regression signal — set-diff the finding keys before/after, where `comm -13 <(keys before) <(keys after)` must be EMPTY. Measured: FIX-Task-60 closed 98 findings and left 135, 102 of them benign-by-design (74 of those the `search_path` hardening above) — driving those to zero would have meant deliberately *weakening* the replay.
4. **Direction must be PROVEN, not inferred — prefer evidence that can be re-run.** "The app probably needs it" is not evidence. Re-runnable forms: a call-site grep for a shipped writer/reader (`rpc('<name>'`, `.from('<table>')`); a live privilege/state query (`has_function_privilege('authenticated', …, 'EXECUTE')`, `pg_class.relrowsecurity`); a definition comparison against the platform it actually runs on; or an existing policy/table analogy (the crate's own sibling precedent). Example: a client-facing RPC's missing `authenticated` grant was provable by combining `has_function_privilege()` with a non-test call-site grep.
5. **Know what the fingerprint CANNOT see, and say so when reporting a residual — a green gate is not "no drift".** The schema fingerprint carries an object's identity/type/security/`proconfig`; it does **not** carry the function **body** (`prosrc`) nor the function/table **ACLs** (`proacl`). Two measured consequences in the same round: (a) a legacy file re-installing an older *body* with an identical signature is invisible (BP-96 rule 8); (b) `dt61_guard_revoke_fn_public` strips PUBLIC/anon/authenticated on every function create, and because every replay file runs after that guard, **21 client-facing RPCs were uncallable on a rebuilt database** (`search_listings`, `get_user_sp_wallet_summary`, `record_policy_acceptance`, the notification reads/preferences) with the gate showing nothing. State the gate's blind spots alongside any residual count.
6. **When the residual is large, the deliverable is the CLASSIFICATION, not a bigger backfill.** Ship a reduced gap plus a verdict for every survivor; explicitly declining a subset with a stated reason is a result, not an omission.
7. **A NEW finding must be ATTRIBUTED before it is reported as a regression — remove your own change and re-measure.** Rule 3's signal (`comm -13` of the key sets) fires on *any* delta, including one your change did not cause, so it tells you a delta exists — never whose it is. In this repo the working tree routinely holds **another session's uncommitted work**, and the replay consumes the **working tree**, so a migration file somebody else added but never committed will move the gate. Before reporting "my change caused a NEW finding" — or acting on it — run the **discriminating re-run**: temporarily move YOUR artifact out of `supabase/migrations/`, re-run `node scripts/migrations/replay-probe.mjs` + `node scripts/migrations/fidelity-check.mjs`, and compare the two reports' key sets. **Identical key sets ⇒ the delta is not yours.** Restore the file, re-replay (confirm the total returns to `<N>/<N>`), and record the attribution in the report. Corroborate with `git status`, and **name any other uncommitted artifact** in the report so the next reader is not misled either. Worked example (FIX-Task-61, 2026-09-18): the gate went **135 → 136**, the single delta being `POLICY|profiles|"Allow phone verification updates"` as a SUBSET MISS; the discriminating run without the new migration file produced a **key-set-identical** report (50 subset / 86 conflicts / 0 unexplained), proving the miss came from `20260918000008_fix_task_59_…_lockdown.sql` — another session's deliberately policy-DROPPING, uncommitted migration awaiting an owner-approved staging apply. Without that one extra replay the delta would have been filed as this task's regression, or worse, "fixed" by restoring a policy that was dropped on purpose (rule 2).

See also: BP-96 (enumerate the gap by fingerprint and re-measure the whole chain — rule 10's inversion is the case this rule's direction test resolves), BP-47 (the latest definition is authoritative — the body-side twin of "check the history before restoring"), BP-99 (order keys, and legacy files that re-declare a shape), BP-78 (audit grants via the LIVE ACL, never greps), BP-75 (live `pg_class.relrowsecurity` is the authority for RLS state), and agent rule 9.1e (a dispatched premise is a claim: "119 missing objects" was reproducible, but the *remedy* stated with it was not).

**Detection checklist:** a task phrased as "backfill everything the schema diff reports" or "drive the finding count to zero" → classify direction per item BEFORE editing a single migration. An item you are about to ADD → grep its name across migrations for a prior `DROP`/removal and read that file's reason first. A residual reported without a per-item verdict → the report is incomplete. A gate reported "green" → confirm you know its blind spots (bodies, ACLs) before concluding "no drift". A gate reports a NEW finding right after YOUR change → run the discriminating re-run (rule 7) and check `git status` for other uncommitted artifacts BEFORE writing it up as a regression; identical key sets with your artifact removed means the delta belongs to someone else, not to you.

## BP-101: Staging Schema Changes Are FILE-FIRST — and the Gate, Not Memory, Decides Whether a Migration Is Safe to Land

**Problem (FIX-Task-63, 2026-09-18):** drift kept being discovered in expensive periodic audits, because the checking was manual and occasional. Three mechanisms, all measured, produced it:

* **Applied-first, file-never.** `fix_task_62_admin_read_lockdown_adapted` was applied to staging in-session because the original file's literal anchor did not match staging's wrapped body text — and **no file of that name has ever existed in the repo** (grep: 0 hits). Live staging was ahead of the chain, permanently, with nothing anywhere saying so.
* **Applied-under-a-different-key.** A ledger row `fix_task_61_item_images_bucket_mime_alignment` exists at version `20260918170406` — a value that is **no filename on disk** (`mcp_supabase_apply_migration` keys its history row by APPLY time). The effect was right; the provenance was unreconstructable.
* **Written-but-never-applied.** The FIX-Task-60 grants file FAILED atomically on staging (it targeted a `search_listings(text,boolean,int)` signature staging does not have), so the chain and staging disagreed in the other direction.

**Rules:**

1. **Write the file FIRST, then apply it.** Never the reverse, and never "apply now, capture it as a migration later" — later is exactly what does not happen. If a change must be adapted because the live body differs from the chain's expectation, the file gets the tolerant form (a whitespace-tolerant anchored match, an exactly-one-match assertion, a partial-apply guard) and is committed before the apply.
2. **An applied change is committed in the SAME session**, with a filename that matches what the ledger recorded. A migration that exists only in a working tree is drift with a short head start — measured: another session's uncommitted `20260918000008_…_lockdown.sql` moved the fidelity gate `135 → 136` with nobody intending it.
3. **Reserve the number before writing the file.** `node scripts/migrations/reserve-number.mjs --next` (or `--block` when a parallel session is live: the `+50000` reserved-block convention). Two files sharing a 14-digit prefix have an **undefined apply order** (BP-99) — this happened for real, and a task had to be renumbered mid-flight. The reservation is machine-checked by the gate's S4.
4. **Let the gate decide — pre-commit is a preview, CI is the enforcement.** `node scripts/migrations/migration-gate.mjs --static` is fast, DB-free and lock-free; `--full` rebuilds from the chain, compares against the committed staging fingerprint, and reports exactly one of `PASS / FAIL / STALE BASELINE / BLOCKED / TIMEOUT`. Only `PASS` exits 0. The bypass is `MIGRATIONS_GATE_BYPASS="<reason>"`, it is loud, it is logged to `tools/reports/gate-bypass.log`, and CI has no bypass at all.
5. **A "staging matches" claim needs a FRESH capture.** The committed snapshot is for repeatability; an old one is not evidence. The gate reports `STALE BASELINE` when it is older than 7 days, when it predates an approved staging DDL — set `lastApprovedStagingDDLAt` in the same session as the apply — or when the fingerprint queries themselves changed (`fpQueriesSha`). Silence from an old capture is not agreement: a 2-day-old snapshot hid a real `nodes.id` default divergence.
6. **Reconcile with the ledger audit, and record exceptions instead of pretending history was clean.** `node scripts/migrations/ledger-audit.mjs` reports three states: `FILE_ONLY` (normal before deployment), `APPLIED_ONLY` (staging has it, no committed file — block and reconcile), `APPLIED_UNCOMMITTED` (applied from the working tree — block closure until committed). Already-known historical exceptions live in `supabase/migrations/tools/ledger-exceptions.json` with a reason, an approver and an **expiry**, so no exception can hide and none stays "temporary" forever.

See also: BP-99 (a filename is an order key), BP-96 (re-run the probe after every migration file you add or edit), BP-80/BP-81 (two-phase provisioning; MCP-applied migrations and their history rows), BP-100 (a new finding must be attributed, and the count is never driven to zero), BP-78 (audit grants from the LIVE ACL).

**Detection checklist:** a proposed staging change with no file → STOP, write the file first. A migration file modified after it was applied → the ledger audit shows it as `APPLIED_UNCOMMITTED`; commit it in the same session. About to create a migration → `reserve-number.mjs --next`, and `--block` if another session is live. A "staging matches" claim → is the snapshot inside its freshness window and newer than the last approved DDL? A gate verdict other than `PASS` → read the verdict name: `STALE BASELINE` means capture, `BLOCKED` means the check could not run (never treat that as green), `TIMEOUT` means the budget was exceeded (never trim the check to fit). Every gate red pair in this task's report was **watched to fail** before it was trusted.
