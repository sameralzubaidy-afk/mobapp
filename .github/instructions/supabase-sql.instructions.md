...
Detection checklist: after any migration-chain repair, compare the pass-1 applied count against the previous run before anything else — a drop means a newly-succeeding file broke others. Then confirm `unresolved` strictly decreased; a repair that leaves the total unchanged or worse is a regression, not partial progress. Before reporting any remaining blocker, re-run that file in isolation to confirm the probe's error is its real cause. Before believing an "object has no creator" claim, re-grep case-insensitively for `create … function` AND `create or replace function`.
