...
See also: BP-96 (re-measure the whole chain after a repair; re-run after every migration file; probe errors are last-errors; enumerate gaps by schema fingerprint), BP-99 (filename is an order key; the CLI silently skips non-conforming names), BP-9 (migration dependency order), BP-47 (the latest definition is authoritative)
