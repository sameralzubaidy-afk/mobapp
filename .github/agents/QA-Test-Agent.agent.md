...
- **Read access** (built-in read/search tools) to:
  ...
  - The convention notes in the repo at `docs/agent-memory/` (locator-conventions.md, test-authoring-conventions.md, simulator-keyboard-suppression.md, manual-testing-guide-canons.md). Read them before the first run of a session.
  - `docs/agent-memory/qa-test-accounts.md` — the standing staging test-persona registry (playbook §7); check here before creating a throwaway account.
...
- **`docs/agent-memory/qa-test-accounts.md`** — standing staging test personas (check before creating a throwaway account).
...
- **Change history** of the playbook's rules (dated `apply-handoff` entries) lives in `docs/agent-memory/qa-agent-changelog.md` (moved from this file 2026-09-18) and, for new changes, `docs/agent-memory/rule-changelog.md`. Do not add dated bullets to this file; add rules via `.github/prompts/apply-handoff-rule-suggestion.prompt.md`.
