...
2. Report: duplicates (the same number defined twice), gaps (a number referenced but never defined), and out-of-order appearances.
   - **Before calling anything a DUPLICATE, print both occurrences and confirm they are not a parent/sub-item pair.** A collision claim is only as good as the pattern that produced it: `^### 5\.[0-9]+` truncates the legitimate `5.47b` to `5.47` and reports every `X`/`Xb` pair as a duplicated section; `**R62 —` misses rule ids defined as `R62a/b/c` and reports them as gaps. The 2026-09-12 audit filed and then **retracted** a "three duplicated §5.x headings" finding for exactly this reason. Capture the full identifier (`^### 5\.[0-9]+[a-z]?`) and eyeball both hits before reporting.
   - **Before calling a number UNASSIGNED, check history, not just the working tree.** "Never allocated" and "was deleted" produce the *same* empty grep today but need opposite fixes (allocate a number vs. restore lost content). Run `git grep -n "<id>" <pre-edit-commit> -- .github/` before declaring a slot free. Worked example (2026-09-12): BP-50/BP-52 returned zero hits at the pre-edit commit — genuinely never allocated, so `UNASSIGNED` was right — while `R79` looked equally empty in the working tree but had been **deleted** by a section rewrite (`§5.70` overwritten for `R80`) and needed restoring, not re-allocating.
