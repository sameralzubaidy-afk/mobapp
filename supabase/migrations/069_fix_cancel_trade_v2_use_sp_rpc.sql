-- filepath: supabase/migrations/069_fix_cancel_trade_v2_use_sp_rpc.sql

-- Mode B: Idempotent rerunnable migration
-- Update cancel_trade_v2 to use credit_sp_for_cancelled_trade RPC and handle SP refund failures gracefully.
...
BEGIN
  ...
  IF v_trade.sp_debit_ledger_entry_id IS NOT NULL THEN
    ...
    IF v_sp_refund_amount > 0 THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        ...
        -- Log: we cannot use RAISE NOTICE in production migrations; return partial success below
        v_sp_refund_ledger_id := NULL;
        v_sp_refund_amount := 0; -- treat as no refund performed
      END;
    END IF;
  END IF;
  ...
  -- 6. Build response: success even if SP refund failed, but include refund details
  RETURN jsonb_build_object(
    'success', true,
    'trade_id', p_trade_id,
    'status', 'cancelled',
...
-- SELECT cancel_trade_v2('b21b1579-64a5-4a85-96ee-b48f91e102f4', '5861bf0e-a925-4a85-96ee-b48f91e10608d', 'Test reason');
