...
-- =============================================================================-- FIX-Task-40 phase 3: the chain's earlier definition of this RPC returns JSON while
-- this one returns JSONB, so CREATE OR REPLACE fails with 42804 ("cannot change return
-- type"). Drop the old signature first. The REVOKE/GRANT pair at the end of this block
-- re-establishes the intended service_role-only ACL after the drop.
DROP FUNCTION IF EXISTS public.admin_force_cancel_trade_db(UUID, UUID, TEXT);
