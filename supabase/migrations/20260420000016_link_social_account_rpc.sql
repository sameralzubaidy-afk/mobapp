...
-- FIX-Task-40 phase 3: `admin_audit_logs` has `action_type` / `entity_type` /
-- `payload` - there is no `action` or `details` column on it (or on any table).
-- This verification SELECT was the last surviving `action` reference.
-- Expected: 1+ rows with action_type = 'link_social_account' after a successful link
SELECT actor_id, action_type, entity_type, payload->>'provider' AS provider, created_at
