...
-- FIX-Task-40 phase 3: 20241227_admin_force_cancel_trade.sql creates this RPC with
-- `RETURNS JSON`; this file changes it to JSONB, and PostgreSQL refuses to change a
-- function's return type through CREATE OR REPLACE (42804). Drop it first - the same
-- prologue the original file already carries. The GRANTs after the body reproduce the
-- ACL this RPC held before the drop; the tighten-to-service_role-only step is a later
-- migration (20260830000010_dev_task_57_rpc_identity_lockdown.sql).
...
-- FIX-Task-40 phase 3: see the DROP prologue above - a dropped function loses its ACLs.
