-- Migration: Fix ADMIN-V2-006 table name error: transactions → trades
-- Date: 2026-03-25
-- Mode: Idempotent rerunnable migration
-- Issue: admin_list_users and admin_get_user_detail referenced non-existent "transactions" table
-- Fix: Change all references from "transactions" to correct "trades" table

-- ===============================================
-- RPC 1: admin_list_users (with trades table fix)
-- ===============================================
...
-- ===============================================
-- RPC 2: admin_get_user_detail (with trades table fix)
-- ===============================================
...
-- 2. Test admin_list_users (replace <admin-user-id> with your admin UUID):
-- SELECT admin_list_users('<admin-user-id>'::uuid, NULL, NULL, NULL, NULL, 1, 20);

-- 3. Test admin_get_user_detail (replace <admin-user-id> and <target-user-id>):
-- SELECT admin_get_user_detail('<admin-user-id>'::uuid, '<target-user-id>'::uuid);

-- 4. Verify no "transactions" references remain:
-- SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname IN ('admin_list_users', 'admin_get_user_detail') AND pg_get_functiondef(oid) LIKE '%transactions%';
-- Expected: 0 rows (should return no results)
