-- Migration: Add user_id search to admin_list_users RPC
-- Mode: Idempotent rerunnable migration
-- Task: Update search on admin users page to support searching by user ID
...
BEGIN
  ...
  -- Check if search term looks like a UUID (for user_id search)
  v_search_is_uuid := p_search IS NOT NULL AND p_search ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$';
