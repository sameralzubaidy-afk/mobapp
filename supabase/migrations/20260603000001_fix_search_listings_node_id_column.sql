-- ================================================================
-- Migration: 20260603000001_fix_search_listings_node_id_column.sql
-- Fix: search_listings RPC node filtering caused false-empty results due to schema drift.
-- Keep p_node_ids for backward compatibility, but do not gate discovery search by node.
-- ================================================================

-- Drop the existing function (V4 signature with 14 params)
DROP FUNCTION IF EXISTS search_listings(TEXT, BOOLEAN, INT, INT, UUID[], TEXT, NUMERIC, NUMERIC, TEXT, TEXT, TEXT, TEXT[], TEXT);

-- Recreate without node gating in discovery search
