-- Hotfix: ADMIN-V2-006 / TC-005 Account Status Filter
-- Mode A: one-time migration
-- Adds explicit deleted user filtering to admin_list_users RPC.
...
BEGIN
  ...
  v_show_deleted_only := (p_account_status = 'deleted');
...
IS 'Admin paginated user list with search/filter support including deleted-only filter via p_account_status = ''deleted''.';
