-- MODULE-15.2 hotfix: CART-006 rpc_cart_get_items
-- Fixes runtime error: function min(uuid) does not exist
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      CASE
        WHEN v_is_subscriber AND i.accepts_swap_points THEN ((i.price * 100)::integer / 2)
        ELSE 0
  ...
  -- NOTE: uuid has no MIN aggregate in this environment; use deterministic array_agg instead.
