...
serve(async (req: Request) => {
  ...
  try {
    ...
    // N2 — Idempotency & Audit: cancellation transitions (best-effort, idempotent keys).
    logFinancialAudit(supabaseClient, {
      mutationType: 'trade_cancelled',
      entityType: 'trade',
      entityId: tradeId,
      actorId: user.id,
      afterState: { reason: reason || 'User requested cancellation', status: 'cancelled', sp_refunded: data.sp_refunded },
      idempotencyKey: `trade_cancelled_${tradeId}`,
    });
    logFinancialAudit(supabaseClient, {
      ...
      entityId: tradeId,
      actorId: user.id,
      afterState: { reason: reason || 'trade_cancelled' },
      idempotencyKey: `tax_voided_${tradeId}`,
    });
    if (trade.stripe_payment_intent_id) {
      logFinancialAudit(supabaseClient, {
        ...
        afterState: {
          ...
          stripe_refund_id: trade.stripe_refund_id ?? null,
          status: 'cancelled',
        },
        idempotencyKey: `payment_cancelled_${tradeId}`,
      });
    }
