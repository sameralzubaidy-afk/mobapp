// File: supabase/functions/release-pending-sp/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    if (error) {
      console.error('[release-pending-sp]', {
        requestId,
        batchSize,
        error: error.message,
      });
      ...
      return jsonResponse(500, {
        success: false,
        error: {
          code: 'RPC_RELEASE_PENDING_SP_FAILED',
    ...
    console.error('[release-pending-sp] unexpected error', {
      requestId,
