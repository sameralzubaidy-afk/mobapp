...
| `qa-scroll-to` | **Works on iOS** (one call scrolls the target into the visible band). **Does NOT scroll on Android** — handler runs, ref resolves, logs `RESULT <id> <x> <y>` in dp with y outside the 825 dp viewport; ScrollView never moves. Use **manual swipes** on Android (3× `mobile_swipe_on_screen`, 800 px, started over non-interactive content). **Later same-day build (FIX-Task-15 item 2, desk-observed via the Metro log, NOT driven by this round):** the tool now reports `RESULT <id> OUT_OF_VIEW <x> <y>` (documented "never a bare success") and emitted `OUT_OF_VIEW NaN NaN` + `measure cannot find view with tag #N` for both Android targets — i.e. **the Android scroll was still a no-op**; only the reporting became honest. Read `OUT_OF_VIEW`/`NaN`/`measure cannot find view with tag` as "tool could not scroll → use the manual swipe", never as an app-side occlusion finding. |
