...
## ⚠️ CORRECTIONS + RESOLUTIONS — FIX-Task-20 (2026-09-12)

> The findings above are **left exactly as reported** (this round's own record and delta are not rewritten).
> The entries below record what live re-verification on 2026-09-12 established, and how each was resolved.

| # | Correction / resolution |
|---|---|
| **F2** | **→ RESOLVED (with a factual correction).** The 34 rows are real and were still `quoted`, but **all 34 carry `tax_amount_cents = 0`** — they are $0-tax (exempt) records, so this was a **pending-COUNT defect, not misreported tax dollars**. Nothing in "Tax Collected" was understated by them. Fixed by `20260912000001` (34 rows → `collected`, `captured_at` from `completed_at`); pre-2026-07-23 completed trades now have `quoted = 0` and the window's pending count dropped 81 → 63 with `pending_tax_cents` unchanged ($162.59). **New, still-open sibling defect:** 9 *post*-migration completed+quoted rows DO carry tax ($80.63 total) but all have `trades.payment_method = 'cash'`, `captured_at IS NULL`, no capture attempt and no capture error — no card capture ever happened, so marking them `collected` would overstate collected tax. They need a void-vs-collect decision and were deliberately excluded from the backfill. |
| **F3** | **→ PARTLY INCORRECT + RESOLVED IN THE OPPOSITE DIRECTION.** The claim that an above-band exempt item "silently falls back to the node rate (i.e. becomes taxable)" is **not what the code does**: `get_applicable_tax_rule` filters on the price band in its `WHERE`, so an out-of-band item returns **zero rows**, and BOTH `calculate_tax()` (`20260830230000` L66-79) and `create-trade-offer` (L851-873, "fail-safe: exempt rather than overtax") treat zero rows as **non-taxable**. Verified live: `calculate_tax(node, 6000, tax_exempt_goods, 6000)` → `$0`. The genuine defect was the **inverse** on the clothing rule: it was `is_taxable = TRUE` with a `$0–$50` band, so clothing **≤ $50 was taxed** and **> $50 came out exempt** — the exact inverse of Connecticut law. Fixed by `20260912000003` + `20260912000004` + `20260912000005`: exempt goods unbounded; clothing split into `$0–$49.99` exempt and `$50.00+` taxable (node rate); the overlap trigger made price-band-aware so disjoint bands may coexist. Verified live (node `Norwalk Central`, tax enabled): $25.00 → $0 · $50.00 → 318¢ · $60.00 → 381¢ · exempt $60.00 → $0. |
| **F4** | **→ RESOLVED.** Live baseline re-measured 2026-07-01..2026-09-12: header 16968 / 175 / 16793 vs `by_jurisdiction` 17143 / 175 / 16968 (gap = 175 = the fully-refunded row). Root cause: the header's `collected` CTE used `tax_status = 'collected'` only, while the breakdown used `IN ('collected','refunded','partially_refunded')`. `20260912000002` applies the owner-approved **GROSS** convention to both: header now **17143 / 175 / 16968**, identical to the breakdown. ⚠️ This changes the semantics of on-screen "Tax Collected" (it now includes the gross tax of transactions later refunded); the P05/O3-C14 guide wording was reconciled in the same pass. |
| **F5** | **→ RESOLVED.** `20260912000003` replaces the raw-UUID message with a named one — verified live: *"A tax rule for \"General Tangible Goods\" (general_tangible_goods) in CT already covers the same price band and date range: \"Updated CT Tangible Goods Rate (v3)\" (2026-09-12 to open-ended)…"*, ids moved to a HINT. |
| **F6** | **→ PARTLY NOT REPRODUCIBLE, then hardened anyway.** The Save button **already** carried `disabled={saving === … || !editValue}` in the current source, so the empty-dropdown button is disabled — the reported "enabled silent no-op" appears to have been observed on an older build. The residual silent early-`return` was replaced with a visible validation error, so *every* entry path now explains itself. (Re-drive before closing.) |
| **F7** | **→ RESOLVED.** `20260912000006` gives the resolver a deterministic total order (`effective_from DESC, version DESC, id ASC`). Live conflict report still shows `general_tangible_goods` with 2 active rules (NY v1 10% + CT v3 6.99%); deactivating the stale NY rule remains an owner DATA decision (deliberately not taken here). |
| **F9** | **→ RESOLVED.** Root cause confirmed: `check_review_reports()` was not `SECURITY DEFINER`, and the only `reviews` UPDATE policy is scoped to the **reviewer**, so the reviewee's in-app report made the trigger's UPDATE match 0 RLS-visible rows and silently no-op (service-role fixtures bypassed RLS, which is why they appeared to work). `20260912000007` makes it `SECURITY DEFINER` (+ `search_path`); `20260912000008` backfills the counters — live: 13 reviews with `report_count > 0`, `has_been_reported = true` ×13, **0 mismatched rows**, `prosecdef = true`. |
| **F10** | **→ NOT REPRODUCIBLE in the current source.** `handleViewChange` already passes `search: null` and the filter bar is remounted on view change, so both the status filter and the search box reset. The real gap was **testability**: the search input had no `data-testid`, which is why the QA leg was typed into the topbar global search instead. A `trades-search-input` test id was added; re-drive W12 before closing. |
| **F12** | **→ RESOLVED (guide-side).** All eight drifts amended in `cross-checked-and-consolidated/MODULE-15.1.2-TradeFlowV2-MANUAL-TESTING.md` with inline 🔄 notes, including the O1-C05 "all items default to general_tangible_goods" claim, the 8-vs-10 category count, the CSV filename/32 columns/status-filtered sum, the conditional Total SP row and the "Bundle Admin Interventions" heading. ⚠️ **Open owner decision:** live `category_tax_mapping` sends the *Clothing product category* to `general_tangible_goods` (not `clothing_footwear`), so the CT clothing exemption does not apply to most clothing listings — flagged, not changed. |
| **F13** | **→ FIXED.** The completed-trade Reviews card + Review CTA was hoisted **above Payment Details** in `TradeTimelineScreen.tsx` (it was the last child of the ScrollView, which is what put its top edge under the tab bar). |
| **F14** | **CONFIRMED (still true).** `click_element` actionability still times out on this admin build; DOM `.click()` via `run_playwright_code` works. Aggregates are still truncatable — pair with `count(*)`. |
