...
| SUB-TC-G01 | Add Stripe Connect payout method (onboarding) | ✅ PASS | PASS | 2026-09-06 | `qa-task37-g01-redrive-2026-09-06` | **PROMOTED from the Remaining ACTIVE table (QA Task 37 — instrumented full re-drive, DT121 name fix landed):** fresh Connect account `acct_1UCgIu4…` created one-tap on qa-payout-seller via the now-AX-exposed Add-modal; **Test/User identity prefilled → NO gov-ID document gate** (currently_due only phone/SSN/TOS — QA Task 36's `individual.verification.document` blocker GONE); hosted-Express drop → terminate → cold deep link `payout-settings?success=true` (DT123) → `continue-onboarding-<id>` CTA → resumed at correct step → Agree and submit → **details_submitted=true, payouts_enabled=true, currently_due=[]**; set primary; funded $5 → real Withdraw → payout `8f085298` (gross 500/fee 26/net 474 processing) + balance→0 + history $4.74/Processing (no real Stripe transfer — trade_id-NULL manual withdrawals have no dispatcher, pre-existing architecture, matches H03 acceptance). Connect account + method + payout cleaned BP-70 |
