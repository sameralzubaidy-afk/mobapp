...
**Generated:** 2026-09-03 · **Last maintained:** 2026-09-05 (QA Task 35 — DT-119 every-branch live verification + payout follow-ups: SUB F08/G10/G06 PARTIAL→PASS — Load More pagination driven on the 25-payout test-seller; requires_action row now carries the live Set Up Payout Method CTA; G05 friendly unverified-copy verified; Batch A/B design-compliance audited on every reachable branch) · **Total canonical cases:** 833
...
| TC-ID | Description | Note / why remaining |
|---|---|---|
| SUB-TC-G01 | Add Stripe Connect payout method (onboarding) | real Stripe Connect onboarding flow — QA Task 34 confirmed boundary (method rows DB-controlled; real onboarding is a real-account follow-up). **QA Task 36 (2026-09-06) PARTIAL on-device drive then SKIPPED (owner-directed):** fresh un-onboarded account `acct_1UCe24KmLuEOxYxB` created on-device (qa-payout-seller) + Express onboarding driven through phone verify → personal details → business details (industry + product description) → Review; phone/SSN/URL cleared (API-verified). **SKIPPED at Stripe ID-document verification** — `currently_due` = `individual.verification.document` + TOS; harness expired the account-link session twice ("Stripe setup session expired"). Root trigger/dev fix: `create-stripe-connect-account` jams the display name into the legal name (`individual.first_name="QA Payout Seller"`, `last_name="Seller"`) → Stripe name+SSN precheck fails → forces gov-ID verification. Re-drive after the name fix (owner may alternatively accept G01 as PARTIAL). Fixture cleaned (account + method row `9e678786` deleted; qa-payout-seller back to 0 methods, active sub intact) |
