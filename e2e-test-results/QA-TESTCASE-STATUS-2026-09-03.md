...
| **SUB** | `MODULE-SUBSCRIPTIONS-PAYOUTS-SPWALLET-MANUAL-TESTING.md` | 100 | 69 | 5 | 3 | 0 | 0 | 0 | 15 | 0 | 2 | **6** |
...
**Guide file:** `cross-checked-and-consolidated/MODULE-SUBSCRIPTIONS-PAYOUTS-SPWALLET-MANUAL-TESTING.md` · **Cases:** 100 · **PASS** 69 · **PARTIAL** 5 · **OPEN** 3 · **DOC-DRIFT** 0 · **SKIPPED** 0 · **RETIRED** 15 · **N/A** 2 · **Remaining (ACTIVE)** 6
...
| SUB-TC-F08 | Payout history Load More pagination (+5) — live | 🟡 PARTIAL | PARTIAL | 2026-09-06 | `qa-task34-payout-e2e-2026-09-05` | DT118 item-4 fix verified on-device (H03 flow): `load-more-button` now has a testID + renders clear of the pill band (y644 on qa-payout-seller). Pagination TAP (+5 on a >5-row seller) not re-driven this round — now drivable by testID (test-seller 25 payouts); QA Task 33's occlusion blocker is cleared |
...
| SUB-TC-G10 | Payout history Load More | 🟡 PARTIAL | PARTIAL | 2026-09-06 | `qa-task34-payout-e2e-2026-09-05` | same as F08 — occlusion + missing-testID blocker cleared by DT118 item 4 (testID + pill-clear); pagination-tap leg still owed on a >5-row seller |
...
| SUB-TC-G05 | Unverified method blocks payout (live: cannot set primary / withdraw) | ✅ PASS | PASS | 2026-09-06 | `qa-task34-payout-e2e-2026-09-05` | scenario `single-unverified` (unverified auto-primary row): Withdraw Now → WithdrawModal opens → Confirm → server reject "Withdrawal Failed / Primary payout method is not verified", NO payout row (DB), balance unchanged. **Doc-drift vs guide**: guide expects client NoMethodModal on Withdraw-Now for unverified-primary; actual = client keys off `is_primary` (opens WithdrawModal) and the RPC enforces `is_verified` at confirm — a clear server-side guard; LOW copy suggestion (see report) |
| SUB-TC-G06 | requires_action payout → "Set Up Payout Method" | 🟡 PARTIAL | PARTIAL | 2026-09-06 | `qa-task34-payout-e2e-2026-09-05` | staged a REAL `requires_action` payout (`39b7e451`, stage-trade with no verified method) → history row renders "Action Required" (red, orange clock) on-device. The guide's **[Set Up Payout Method] button is NOT on the live PayoutSettings history card** (source-proven — PayoutHistoryCard has no CTA; guide targets the dead SellerEarningsScreen) — doc-drift + UX gap (no in-row action to resolve requires_action) |
