...
| SUB-TC-D05 | Reactivate from cancelled state | ✅ PASS | PASS | 2026-09-06 | `qa-task34-payout-e2e-2026-09-05` | QA33 discovery (sp_wallets stuck grace after reactivation) FIXED + live-verified by DEV-TASK-118 (2026-09-06, fresh disposable `bb862192` reset to grace → real renewal → EF `sp_wallet_unfrozen:true` → `sp_wallets.state='active'`, grace cleared; disposable deleted BP-70). **QA Task 34 cross-ref (not independently re-driven)**: deployed `renew-subscription` EF confirmed unchanged at HEAD `5ef735fa` (DT118) with the step-9 service-role `rpc_set_sp_wallet_state('active')` + loud-failure (`sp_wallet_unfrozen:false`) path. Fresh independent re-drive = dedicated real-Stripe-disposable fixture session (R41 discipline) |
...
| TC-ID | Description | Note / why remaining |
|---|---|---|
| SUB-TC-G01 | Add Stripe Connect payout method (onboarding) | real Stripe Connect onboarding flow — QA Task 34 confirmed boundary (method rows DB-controlled; real onboarding is a real-account follow-up, recorded in `qa-fixture-payout-runbook.md` Known boundaries) |
...
| SUB-TC-N06 | ContinueKidsClub trial-ending urgency badge | needs a ≤7-day-trial mobile persona (none on staging) — **QA Task 35 (DT-119)**: trial-branch styles source-audited canonical (urgency pill = theme warning #FFF3E0 bg / #FFA726 text; trial>7d renders no pill by design; non-trial free/grace/expired show the positive green-tint pill, verified on-device for those states). The trial≤7d + trial>7d ON-DEVICE renders still need a `test-trial`-class fixture (guide's test-trial@ actor is unprovisioned on staging; provisioning gap) |
