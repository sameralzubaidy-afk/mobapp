...
| **ADM** | `MODULE-ADMIN-PORTAL-MANUAL-TESTING.md` | 160 | 130 | 25 | 1 | 0 | 1 | 0 | 0 | 0 | 0 | **3** |
...
**Guide file:** `cross-checked-and-consolidated/MODULE-ADMIN-PORTAL-MANUAL-TESTING.md` · **Cases:** 160 (canonical) · **PASS** 130 · **PARTIAL** 25 · **OPEN (FAIL)** 1 · **DOC-DRIFT** 0 · **SKIPPED** 1 · **Remaining (NEVER RUN)** 3 · (+8 supplementary sub-case rows in Completed — see roll-up note)
...
| ADM-TC-I03 | Resolve dispute — Complete | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | fe3924ee → Resolve→Complete → completed + resolved_seller + actor 1a546991 + seller_payouts row 50c024d1 (pending $16.81) |
| ADM-TC-I04 | Resolve dispute — Refund | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | 6a1f9d94 → Resolve→Refund → cancelled + resolved_buyer + actor + stripe_refund_id=cancelled_pi_3UAysJ (auth-hold cancel) + tax voided |
...
| ADM-TC-D02 | Create / edit category + SP multiplier | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | disposable 8dfa9245 created + multiplier 1.15 (DB) + Live Preview 57 SP |
...
| ADM-TC-D05 | Icon / badge upload | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | Icon & Badge fields render; real icon upload → icon + icon_url storage object (DB) |
| ADM-TC-D06 | SP spending cap % | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | cap 60 → DB; Live Preview 30 SP |
| ADM-TC-D07 | SP redemption cap | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | redemption 40 → DB |
| ADM-TC-D08 | Drag-and-drop reorder | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | drag handles render + reorder API route source-verified; HTML5 drag not drivable in embedded driver |
| ADM-TC-D09 | Bulk actions | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | bulk Deactivate/Activate on 2 disposables DB-verified; menu Activate/Deactivate/Delete/Export CSV (no (hides items)/(some have items) suffixes — doc-drift) |
| ADM-TC-D10 | Delete category + guards | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | items guard (Books 15/Toys 1038/Shoes 1 disabled "Cannot delete: N items or system category"); empty delete exact confirm; Other disabled |
...
| ADM-TC-E02 | Add / edit node | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | disposable aeffbaa5 created (zip autolookup) + edited (DB) |
| ADM-TC-E03 | Deactivate node w/ members warning | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | Diag Test Node members-warning exact (10 members) → deactivate (DB) → reactivate (DB) |
| ADM-TC-E04 | Node settings radius validations | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | valid 10→12→10 (DB) + invalid 150 blocked exact msgs; reverted |
| ADM-TC-E05 | ZIP waitlist queue + filter | ✅ PASS | PASS | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | metrics 14/14/0/0 DB-exact; search ZIP filters; Joined empty state exact |
...
| ADM-TC-O04 | Request details (screenshot deleted note) | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | ID details surface (O01–O03 PASS); "screenshot deleted note" state not present this round |
...
| ADM-TC-P02 | Edit badge / toggle | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | /badges list + Edit affordance (P01 PASS'd toggles QA29); edit commit not re-driven |
| ADM-TC-P03 | Manual award badge | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | award surface not on list view; not driven |
...
| ADM-TC-R03 | Publish FAQ/education content | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31-adm-near-total-2026-09-04` | Publish affordance; content commit not driven |
...
| ADM-TC-C12 | Individual filter controls | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31t-dt111-adm-2026-09-04` | query (Results 16 for "QA Bundle Fixture"), status=pending (81 = DB exact), SP-eligible (456 = DB exact) PASS server-side. **DEGRADED:** category (Toys → Results 9 vs 1078 DB) + seller-email (test-seller → Results 20 vs 274 DB) — admin_search_listings_v2 on staging is the 5-arg legacy signature (DT97 migration 20260903000001 dropped p_category/p_seller_email), UI falls back to client-side filtering of only the newest ≤20-row page with totalCount=page length. Finding: restore the 7-arg signature. Also Paused absent from status dropdown (439 paused items exist) |
...
| ADM-TC-L07 | SP Wallet state RPC — get_user_sp_wallet_summary returns wallet_state | 🟡 PARTIAL | PARTIAL | 2026-09-04 | `qa-task31t-dt111-adm-2026-09-04` | RPC/backend leg PASS (wallet_state returned + consumed by AuthContext → frozen banner; DB state flips verified). **Mobile SP-disabled leg gap:** with a frozen wallet, the Make Offer (TradeOfferScreen) surface from an Accept-SP listing still shows the "ADD SP OFFER" input ENABLED ("Max 12 SP") — only the WalletWarningBanner renders; TradeOfferScreen gates SP on isSubscriber+accepts_swap_points+maxSpToUse (no canSpendSP/editable gate, unlike TradeInitiationScreen). Recommend a canSpendSP gate on sp-amount-input |
