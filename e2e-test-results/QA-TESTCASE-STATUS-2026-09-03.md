...
| **SUB** | `MODULE-SUBSCRIPTIONS-PAYOUTS-SPWALLET-MANUAL-TESTING.md` | 100 | 76 | 2 | 3 | 0 | 0 | 0 | 15 | 0 | 2 | **2** |
...
**Guide file:** `cross-checked-and-consolidated/MODULE-SUBSCRIPTIONS-PAYOUTS-SPWALLET-MANUAL-TESTING.md` · **Cases:** 100 · **PASS** 76 · **PARTIAL** 2 · **OPEN** 3 · **DOC-DRIFT** 0 · **SKIPPED** 0 · **RETIRED** 15 · **N/A** 2 · **Remaining (ACTIVE)** 2
...
### Remaining test cases — ACTIVE (2) — QA Task 39 state (G01 struck placeholder + N06)
...
| SUB-TC-N06 | ContinueKidsClub trial-ending urgency badge | needs a ≤7-day-trial mobile persona (none on staging) — **QA Task 35 (DT-119)**: trial-branch styles source-audited canonical (urgency pill = theme warning #FFF3E0 bg / #FFA726 text; trial>7d renders no pill by design; non-trial free/grace/expired show the positive green-tint pill, verified on-device for those states). **QA Task 36 (2026-09-06) re-confirmed:** `test-trial@kidsmarketplace.test` still unprovisioned on staging (read-back empty). **🟢 OWNER APPROVAL GRANTED 2026-09-06** for DT120 Phase 2 (staging ensure + on-device ≤7d/>7d render) — approved run plan recorded in `/memories/repo/qa-dt120-trial-approval-2026-09-06.md`; **execution scheduled for a dedicated QA session** (owner: "not through the approval chat"). Plan: `qa:r41-trial -- ensure` (5d) → on-device ≤7d urgency-pill check via `qa-login-as?persona=test-trial` → `continue-kids-club` → `qa:r41-trial -- ensure --days-remaining 14` → on-device >7d no-pill check → `qa:r41-trial -- reset`. N06 not closable in the QA Task 36 session (approval arrived after). **QA Task 39 (2026-09-06) — the DT120 Phase-2 staging run WAS attempted (this dedicated session) but BLOCKED on a fixture defect:** `npm run qa:r41-trial -- ensure` fails at the subscriptions upsert — `Could not find the 'trial_started_at' column of 'subscriptions'` (staging column is `trial_start_date`, verified via `information_schema`; every other written column exists). Bug in TWO places in `scripts/qa/r41-trial-fixture.mjs`: the upsert object (~L213) + the `printStatus` select (~L258) reference `trial_started_at`. Execution-only → **one-line fix `trial_started_at → trial_start_date` recommended as a dev follow-up; then re-run the approved plan**. ContinueKidsClubScreen trial-branch render logic source-audited correct (L182-210: urgency badge when trial && days≤7 && >0; `showDefaultTrialBadge = !isTrialSubscription`). Half-provisioned persona reset to 0 residue (DB-verified). |
