...
- **Fixture built (Phase 1 code):** `npm run qa:r41-first-trade -- create` provisions a standing free persona `qa-first-trade` (`a1234567-…-000000000017` — see the UUID note below, `qa-first-trade@kidsmarketplace.test`, `TestFirstTrade123!`, registered in `r41-common.mjs PERSONAS` + `qaPersonas.ts`) with a saved Stripe test card (MASTERCARD •••• 4444, `tok_mastercard`) and NO `trial|active` subscription (signup trigger `status='free'` row only), so `profiles.fee_state` stays `no_completed_trade` / `completed_trade_count` 0 — plus one tagged Accept-SP item on test-seller.
- **Phase 2 — EXECUTED (owner-approved 2026-09-05):** `create` ✅ (persona `a1234567-0000-0000-0000-000000000014`, card `pm_1UCIuV4I6kCJlvXoSovEnPNm` / customer `cus_VCiLNyxeb4WDix`, item `89a6573a-ae66-488e-9805-6979dc6e5c27` $25 Accept-SP on test-seller) → `verify-fee` ✅ (`profiles.fee_state=no_completed_trade` `completed_trade_count=0`; `fn_get_buyer_fee_for_checkout` → **fee_cents=149** = `buyer_fee_first_trade_cents`).
- **🔴 UUID COLLISION — FIXED 2026-09-12 (FIX-Task-23 item 1):** the original persona id `a1234567-…-000000000014` was **also** assigned to the DEV-TASK-96 no-conversation persona (`test-noconvo` — `seed-staging-data.ts` `noConversationUser`), so `create` could never succeed again with the bare GoTrue error *"Database error creating new user"*. That is why K02/F08's first-trade leg read "not provisioned" across QA rounds (the provisioning path was **broken**, not merely unrun). Both locations now use `a1234567-…-000000000017`: `scripts/qa/lib/r41-common.mjs` `PERSONAS['qa-first-trade']` **and** `scripts/qa/r41-first-trade-free.mjs` `PERSONA.id` (keep them in sync). `findOrCreatePersona()` now fails loudly, naming the occupying email, if the fixed UUID is ever taken again. **Two-phase re-provision required:** the persona must be re-created on staging (`npm run qa:r41-first-trade -- create`) before the F08/K02 first-trade leg can be driven.
