...
Notes:
- **FIX-Task-66 item 11 (2026-09-18): the documented password value is NOT authoritative — never trust it.** MSG Round 2 hit "Invalid login credentials" with a stale documented value while the account, its RBAC row and sign-in itself were all fine (an admin-portal login failure is therefore NOT evidence of an app defect until the credential is validated). The literal was removed from this registry to eliminate the trap. Before an admin/Playwright run, **validate the credential with a GoTrue password-grant** and read the live value from `test-automation/trade-flow-v2/.env` (`PLAYWRIGHT_ADMIN_PASSWORD`) or `p2p-kids-admin/.env.local`. If the grant fails, report a stale-credential setup gap.
