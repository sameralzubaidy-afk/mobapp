...
- Smoke: (manual)
  ...
  - **EDU-004-ONBOARDING-CAROUSEL (2026-05-03):** Trading education onboarding carousel (first-run only)
    - Module: MODULE-18-TRADING-EDUCATION (TASK EDU-004)
    - Scope:
      - `p2p-kids-marketplace/src/screens/onboarding/OnboardingScreen.tsx`
      - `p2p-kids-marketplace/src/components/onboarding/OnboardingCarousel.tsx`
      - `p2p-kids-marketplace/src/components/onboarding/OnboardingScreenCard.tsx`
      - `p2p-kids-marketplace/src/data/onboarding-screens.ts`
      - `p2p-kids-marketplace/src/navigation/AppNavigator.tsx` (RootNavigator integration)
    - Features:
      - 5-screen swipeable carousel shown on first app open (after auth, before main tabs)
      - Progress dots (filled = current; ghosted = others)
      - Skip button on every screen → marks `onboarding_skipped_at` and navigates to Home
      - Get Started button on screen 5 → marks `onboarding_completed_at` and navigates to Home
      - DB content override: screens 2-5 pull body from `education_sections` table (fallback to static)
      - First-run gating via `shouldShowOnboarding(userId)` (checks both timestamps are NULL)
      - Analytics events: `onboarding_start`, `onboarding_complete`, `onboarding_skip`
    - DB Dependencies:
      - `user_profiles.onboarding_completed_at` (MODULE-18 EDU-001)
      - `user_profiles.onboarding_skipped_at` (MODULE-18 EDU-001)
      - `education_sections` table with published sections (MODULE-18 EDU-001)
      - `education_analytics` table (MODULE-18 EDU-001)
    - Tests:
      - Unit: `src/components/onboarding/__tests__/OnboardingCarousel.test.tsx` (coverage ≥85%)
      - Unit: `src/screens/onboarding/__tests__/OnboardingScreen.test.tsx` (coverage ≥85%)
      - Integration: `e2e/onboarding-carousel.integration.test.ts` (RUN_SUPABASE_E2E=true)
      - Maestro: `.maestro/onboarding-carousel.yaml` (3 flows: complete, skip, swipe navigation)
      - Manual: `TASK-EDU-004-MANUAL-TESTING.md` (10 test cases + accessibility)
    - Prerequisites (SQL):
      - Run migrations: `20260420000018_create_education_sections.sql` through `20260420000021_education_publish_rpcs.sql`
      - Seed education sections for onboarding content
      - Fresh test user with NULL onboarding timestamps
    - Validation:
      - `npm run typecheck` (must pass)
      - `npm run lint` (must pass)
      - `npm test -- --testPathPattern=onboarding` (unit tests green)
      - `RUN_SUPABASE_E2E=true npm test -- --testPathPattern=onboarding-carousel.integration` (integration tests pass)
      - `npm run test:maestro:ios` -- onboarding-carousel.yaml (all 3 flows pass)
      - Manual testing per `TASK-EDU-004-MANUAL-TESTING.md` (all 10 test cases pass)
