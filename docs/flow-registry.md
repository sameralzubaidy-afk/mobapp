...
- **Fix 3 — C03 linked-provider fixture (FLOW-01):** standing `qa-linked-provider@kidsmarketplace.test` (id `a1234567-…-00000000000e`, password `TestLinked123!`) with a completed profile + ONE genuinely linked Google identity (operator SQL `INSERT INTO auth.identities`, synthetic provider id `qa-linked-provider-google`), provisioned by `npm run seed:linked-provider-fixture` (`scripts/provision-linked-provider-fixture.ts`). Unlink flow (confirmation + success) testable via email/password login. Last-method guard (C03 step 2) documented as requiring a social-only persona (count=1) — natural fixture is C07 once a real identity is attached; no C04 fixture touched.
