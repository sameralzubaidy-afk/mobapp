...
- Smoke: (manual)
  ...
  - **STARTER-PACK-PARITY-FLAGGED-APPROVAL (2026-08-21):** Flagged-queue approval now grants Starter-Pack eligibility identically to standard approval
    ...
    - Pre-existing blocker found (NOT caused by this change): `issue_starter_pack` fails with `cannot cast jsonb string to type integer` when `sp_config.starter_pack_amount` is a JSON string (staging seed stores `"10"` as a string) — blocks the positive award case on staging for BOTH paths. Small follow-up fix needed (safe-int cast or JSON-number seed).
