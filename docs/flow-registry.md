...
- Smoke: (manual)
  ...
  - **LISTING-APPROVAL-NOTIFICATION-RESTORE (2026-08-21):** Restored the seller "Listing Approved" notification that the R8 rewrite accidentally dropped
    ...
    - Related (INVESTIGATED, NOT changed — pending product decision): flagged-queue approve via `/api/admin/items/[id]/status` (SAFETY-008 path, `mobappadmin` repo) bypasses `admin_approve_listing` — direct service-role `items.update` sets `status='available'` without `approved_at`/`approved_by`, moderation gate, starter pack, `admin_activity_log`, or seller notification. Seller IS notified for flagged/rejected/needs_edits by DB trigger `on_item_status_change_notify_seller`, but NOT for approval. With the AI gate enabled, the RPC would hard-block flagged items (`MODERATION_BLOCKED_FLAGGED`), so the direct path is currently the only human-override route for flags — the split looks deliberate for that reason but is under-built on approval metadata/notification. Product owner to decide: route flagged-queue approval through `admin_approve_listing` (with a human-override flag) or make the direct path complete in place.
