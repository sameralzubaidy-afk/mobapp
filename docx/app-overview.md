...
13. **FLOW-30**: Global App Shell — read-only node-chip header with bell/chat/avatar cluster, floating pill bottom nav (Home / Discover / Sell / Trades / Basket), Trades tab (active/history + active-count badge), and Home composer bar that pre-fills the New Item Title (2026-08-11)
14. **N2 (Cross-Cutting)**: Idempotency & audit — every payment/SP/fee/tax transition is retry-safe (no double-charge / double-issue / double-log) and logged to the unified `financial_audit_log` journal; Stripe PaymentIntents use deterministic idempotency keys, and `trades.stripe_payment_intent_id` / `trade_refunds.stripe_refund_id` are unique (SYSTEM_REQUIREMENTS_V2 §8B)
...
6. **Global app shell (2026-08-11)**: Read-only market context (node chip), floating pill bottom nav, and a one-tap Home composer that jumps straight into listing creation with the Title pre-filled
...
- Composer → New Item: 1 tap (+ optional camera shortcut) with Title pre-filled
