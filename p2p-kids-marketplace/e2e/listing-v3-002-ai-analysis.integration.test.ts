...
const extractInvokeErrorMessage = async (error: unknown): Promise<string> => {
  ...
  const maybeContext = (error as { context?: { json?: () => Promise<any>; text?: () => Promise<string> } }).context;
...
describeE2E('LISTING-V3-002: AI Analysis Integration Tests', () => {
  ...
  test('TC-001: analyze-item-image returns valid AIAnalysisResult', async () => {
    ...
    if (result.title) {
      ...
      expect(result.title.confidence).toBeGreaterThanOrEqual(0.40);
    }
    
    if (result.category) {
      expect(result.category.value.label).toBeDefined();
      expect(result.category.confidence).toBeGreaterThanOrEqual(0.40);
    }
    
    if (result.condition) {
      expect(['new', 'like_new', 'good', 'fair', 'worn']).toContain(result.condition.value);
      expect(result.condition.confidence).toBeGreaterThanOrEqual(0.40);
    }
    
    console.log('✅ TC-001 PASSED: Single image analysis succeeded');
    console.log('  Fields extracted:', Object.keys(result).filter(k => k !== 'rawLabels'));
  }, TEST_TIMEOUT);

  test('TC-002: analyze-item-image filters fields below MIN_CONFIDENCE', async () => {
    if (shouldSkipCase()) return;
    ...
    const fieldsToCheck: Array<keyof AIAnalysisResult> = [
      'title', 'category', 'condition', 'brand', 'color', 'age_group', 'gender'
    ];
    
    for (const field of fieldsToCheck) {
      const value = result[field] as any;
      if (value && typeof value === 'object' && 'confidence' in value) {
        expect(value.confidence).toBeGreaterThanOrEqual(0.40);
      }
    }
  ...
  test('TC-003: analyze-item-image handles requestFields parameter', async () => {
    if (shouldSkipCase()) return;
  ...
  test('TC-004: batch-analyze-items processes multiple items', async () => {
    if (shouldSkipCase()) return;
    ...
    console.log(`  Success rate: ${data.totalProcessed - data.totalFailed}/${data.totalProcessed}`);
  }, TEST_TIMEOUT * 2); // Double timeout for batch

  test('TC-005: batch-analyze-items respects MAX_CONCURRENCY', async () => {
    if (shouldSkipCase()) return;
  ...
  test('TC-006: analyze-item-image handles missing photoUrl', async () => {
    if (shouldSkipCase()) return;
  ...
  test('TC-007: batch-analyze-items handles invalid items array', async () => {
    if (shouldSkipCase()) return;
  ...
  test('TC-008: analyze-item-image handles invalid photo URL gracefully', async () => {
    if (shouldSkipCase()) return;
  ...
  test('TC-009: batch-analyze-items partial failure tolerance', async () => {
    if (shouldSkipCase()) return;
