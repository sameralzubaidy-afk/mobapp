...
export async function extendTrial(
  ...
  try {
    console.log('[trialExtension] 📅 Extending trial for user:', userId, 'via referral:', referralUserId);
...
export async function getTrialExtensionStats(userId: string): Promise<{
  ...
  try {
    ...
    if (configError) {
      console.error('[trialExtension] ❌ Failed to fetch max extensions config:', configError.message);
      return null;
    }
 ...
 */
export async function getTrialExtensionHistory(userId: string): Promise<Array<{
  id: string;
  event_type: string;
  metadata: {
