...
export const verifyPhoneCode = async (
  ...
  try {
    ...
    const result = (Array.isArray(rpcResult) ? rpcResult[0] : rpcResult) as { success: boolean; message?: string } | null;
    ...
    try {
      ...
      if (fnData && (fnData.error || fnData?.status >= 400)) {
        ...
        const errorMessage = typeof msg === 'string' ? msg : (msg?.error && typeof msg.error === 'string' ? msg.error : '');
        
        if (errorMessage.includes('missing service role key') || errorMessage.includes('service role')) {
