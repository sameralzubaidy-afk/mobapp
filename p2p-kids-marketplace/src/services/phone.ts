...
export const requestPhoneVerification = async (
  userId: string,
  phone: string
): Promise<{ success: boolean; code?: string; error?: any }> => {
  try {
    ...
    console.error('requestPhoneVerification exception:', e);
...
export const verifyPhoneCode = async (
  ...
  try {
    const { data: rpcResult, error: rpcError } = await supabase.rpc('verify_phone_code', { p_user_id: userId, p_code: code });
    if (rpcError) {
      ...
      if (rpcError.code === '42702') {
        return { success: false, error: new Error('Server function ambiguity error: please run the verify_phone_code migration to fix the function on the database.') };
      }
    ...
    try {
      const invokeRes = await supabase.functions.invoke('auth-update-phone', { body: { user_id: userId, phone } });
      ...
      if (fnData && (fnData.error || fnData?.status >= 400)) {
        ...
        if (typeof msg === 'string' && msg.includes('missing service role key') || (msg && msg.error && typeof msg.error === 'string' && msg.error.includes('service role')) ) {
          console.warn('auth-update-phone function not configured; falling back to updating profiles table');
          const { error: profileErr } = await supabase.from('profiles').update({ phone, phone_verified: true, phone_verified_at: new Date().toISOString() }).eq('user_id', userId);
          if (profileErr) {
            console.error('Fallback profile update failed:', profileErr);
            return { success: false, error: profileErr };
          }
          return { success: true, message: 'Phone verified and saved to profile (auth not updated: service role missing)' };
        }
        return { success: false, error: fnData.error || fnData };
      }
      return { success: true, message: 'Phone verified and updated' };
    } catch (e) {
      console.error('Error invoking auth-update-phone function:', e);
      return { success: false, error: e };
    }
  } catch (e) {
    console.error('verifyPhoneCode exception:', e);
    return { success: false, error: e };
  }
};
