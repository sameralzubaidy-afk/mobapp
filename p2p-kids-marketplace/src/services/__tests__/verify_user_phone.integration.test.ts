...
describeSupabase('verify_user_phone RPC', () => {
  ...
  const seedVerificationRow = async (): Promise<void> => {
    ...
    const { error: v3Error } = await service.from('phone_verification_codes').insert({
      user_id: testUserId,
      phone,
    ...
    const { error: legacyError } = await service.from('phone_verification_codes').insert({
      user_id: testUserId,
      phone,
      code: '999999',
      verified: true,
    ...
    if (legacyError) {
      console.warn(
        '[verify_user_phone.integration] Could not seed verification row; continuing with RPC call.',
        {
  ...
  beforeAll(async () => {
    ...
    if (!created?.userId) {
      throw new Error(
        'Failed to create confirmed test user. Set SUPABASE_SERVICE_ROLE_KEY for integration tests.'
      );
    }
