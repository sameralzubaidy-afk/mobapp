...
//
// DEPRECATED (Dev Task 86, 2026-09-02): This screen is DEAD — it is NOT registered
// in AppNavigator and has NO live navigation callers, so it is unreachable from the
// app. Its only references are its own unit test and archived/historical docs. The
// LIVE consolidated payout surface is PayoutSettingsScreen (balance hero + method
// card + "Withdraw Now" modal). Do NOT add new navigation or callers. Removal
// candidate: safe to delete this file together with
// src/__tests__/screens/PayoutDashboardScreen.test.tsx.
...
export default function PayoutDashboardScreen() {
  ...
  // FIX-Task-47 item 3 (2026-09-16): same unconditional-Load-More class as Payout
  // Settings. NOTE: this screen is currently UNREGISTERED in AppNavigator (its only
  // caller was itself dead), so this is defensive class-consistency hardening with
  // no user-facing effect today.
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [balanceData, payoutsData, methodsData] = await Promise.all([
        getSellerBalance(),
        // FIX-Task-47 item 3: +1 probe so `hasMore` is provable without a count query.
        getRecentPayouts(11),
        listPayoutMethods(),
      ]);
      setBalance(balanceData);
      setHasMorePayouts(payoutsData.length > 10);
      setPayouts(payoutsData.slice(0, 10));
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const more = await getRecentPayouts(newLimit + 1);
      setHasMorePayouts(more.length > newLimit);
      setPayouts(more.slice(0, newLimit));
    } catch (err: any) {
        ...
        <View style={styles.sectionCard}>
          ...
          {payouts.length === 0 ? (
            ...
            <>
              ...
              {/* FIX-Task-47 item 3 (2026-09-16): gated on `hasMorePayouts` so the last
                  page ends deliberately instead of showing a dead control. */}
              {hasMorePayouts ? (
                <TouchableOpacity
                ...
                <Text style={styles.endOfHistoryText} testID="payout-history-end">
