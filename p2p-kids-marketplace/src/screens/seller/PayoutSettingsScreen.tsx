...
export default function PayoutSettingsScreen() {
  ...
  // DT-122 (I-2): id of the Stripe Connect method whose hosted-onboarding link
  // is being generated by the "Continue Onboarding" card action (spinner).
  const [resumingOnboardingId, setResumingOnboardingId] = useState<string | null>(null);
  ...
  // directly from the method card (no Add-Method detour). The method already
  // has a Stripe account, so we only mint a fresh account link.
  const handleContinueOnboarding = async (method: SellerPayoutMethod) => {
    if (!method.stripe_account_id || resumingOnboardingId) return;
    try {
      setResumingOnboardingId(method.id);
      const { url } = await createStripeAccountLinkUrl(method.id);
      ...
      captureException(error, {
        tags: { screen: 'PayoutSettingsScreen', action: 'resume_stripe_onboarding' },
      });
      Alert.alert(
        'Unable to Resume Onboarding',
        "We couldn't reopen your Stripe onboarding. Pull down to refresh and try again."
      );
    } finally {
      setResumingOnboardingId(null);
    }
  };
        ...
        {methods.length > 0 ? (
          <>
            {methods.map((method) => {
              ...
              // DT-122 (I-2): an incomplete Stripe Connect method with a real
              // account gets a direct "Continue Onboarding" resume action so a
              // dropped hosted-onboarding session resumes in one tap.
              const showContinueOnboarding =
                method.method_type === 'stripe_connect' &&
                !!method.stripe_account_id &&
                !method.stripe_onboarding_complete;

              return (
                <View key={method.id} style={styles.methodCardWrap}>
                  <View style={styles.methodCard} testID={`method-card-${method.id}`}>
                  ...
                  {showContinueOnboarding && (
                    <TouchableOpacity
                      style={styles.continueOnboardingButton}
                      onPress={() => handleContinueOnboarding(method)}
                      disabled={resumingOnboardingId === method.id}
                      testID={`continue-onboarding-${method.id}`}
                      accessible
                      accessibilityRole="button"
                      accessibilityLabel="Continue Stripe onboarding"
                    >
                      {resumingOnboardingId === method.id ? (
                        <ActivityIndicator size="small" color="#FFFFFF" />
                      ) : (
                        <Clock size={16} color="#FFFFFF" weight="bold" />
                      )}
                      <Text style={styles.continueOnboardingButtonText}>
                        {resumingOnboardingId === method.id
                          ? 'Opening onboarding…'
                          : 'Continue Onboarding'}
                      </Text>
      ...
      <View style={styles.withdrawModalContent}>
        ...
        <View style={styles.modalActions}>
          ...
          <TouchableOpacity
            ...
            accessible
            accessibilityRole="button"
            accessibilityLabel={withdrawing ? 'Processing' : 'Confirm Withdrawal'}
          >
            {withdrawing ? (
...
interface AddPayoutMethodModalProps {
  ...
  /** DT-121 (item 4): true when the seller is resuming an existing,
   *  not-yet-complete Stripe Connect onboarding — surfaces the phone
   *  re-verification hint on the redirect alert. */
      ...
      switch (selectedType) {
        case 'stripe_connect':
          ...
          try {
            ...
            // Mint the hosted-onboarding account link (DT-122 I-2: shared with
            // the method-card "Continue Onboarding" action so a dropped session
      ...
      <View style={styles.modalContent}>
        ...
        <View style={styles.methodTypeSection}>
          <TouchableOpacity
            ...
            testID="add-method-type-stripe-connect"
            accessible
            accessibilityRole="button"
            accessibilityLabel="Stripe Connect — Bank deposits via Stripe"
          >
          ...
          <TouchableOpacity
            ...
            testID="add-method-type-paypal"
            accessible
            accessibilityRole="button"
            accessibilityLabel={`PayPal — ${payoutFeeSummary}`}
          >
          ...
          <TouchableOpacity
            ...
            testID="add-method-type-venmo"
            accessible
            accessibilityRole="button"
            accessibilityLabel={`Venmo — ${payoutFeeSummary}`}
          >
        ...
        {selectedType === 'paypal' && (
          <View style={styles.inputSection}>
            ...
            <TextInput inputAccessoryViewID={KEYBOARD_DONE_ACCESSORY_ID}
              ...
              testID="add-method-paypal-email"
            />
          </View>
        )}
        ...
        {selectedType === 'venmo' && (
          <View style={styles.inputSection}>
            ...
            <TextInput inputAccessoryViewID={KEYBOARD_DONE_ACCESSORY_ID}
              ...
              testID="add-method-venmo-handle"
            />
          </View>
        )}
        ...
        <View style={styles.modalActions}>
          ...
          <TouchableOpacity
            ...
            disabled={submitting}
            testID="add-method-submit"
            accessible
            accessibilityRole="button"
            accessibilityLabel={submitting ? 'Processing' : 'Add Method'}
          >
            {submitting ? (
              <ActivityIndicator size="small" color="#fff" />
            ) : (
...
const styles = StyleSheet.create({
  ...
  // ── Payout method card ─────────────────────────────────────────────────────
  // DT-122 (I-2): each method card is wrapped in methodCardWrap so an incomplete
  // Stripe Connect method can render a full-width "Continue Onboarding" CTA
  // directly beneath its card; the group spacing lives on the wrapper.
  methodCardWrap: {
    marginBottom: 12,
  },
  methodCard: {
  ...
  // DT-122 (I-2): resume CTA shown beneath an incomplete Stripe Connect method.
