    ...
    <View style={styles.payoutCard}>
      ...
      {item.failure_reason && (
        <View style={styles.failureReason}>
          {/* FIX-Task-47 item 5 (2026-09-16): same raw-column leak class as Payout
              Settings — `failure_reason` is free text written by provider webhooks
              and by QA/operator scripts, so it is never rendered verbatim. */}
          <Text style={styles.failureText}>
            ⚠️ {getFriendlyPayoutFailureReason(item.failure_reason)}
          </Text>
        </View>
      )}
