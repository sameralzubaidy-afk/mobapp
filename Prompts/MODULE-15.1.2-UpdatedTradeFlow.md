...
## 1. Description
This module updates the Trade Flow (FLOW-08) to require a payment authorization hold when a buyer makes an offer. This ensures buyer commitment, reduces spam/non-serious offers, and protects market credibility. 

Key changes:
- Buyers must have a valid payment method to make an offer.
- A hold is placed on the buyer's card (cash + platform fee) and their Swap Points (SP) when making an offer.
