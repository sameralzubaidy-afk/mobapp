...
| TRD-TC-E01 | Buyer opens Report a Problem modal | 1 | PASS | 2026-08-29 | `qa-task6-e-reverify-2026-08-29` (QA Task 6 full re-verify) | ✅ PASS (occlusion fixed DEV-TASK-40 — button clears pill; modal opens with 5 reasons; full submit → amber dispute banner; Report Problem hidden during dispute) |
...
| TRD-TC-M05 | Cannot add own item to cart | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (own listing → Add to Cart hidden on-device, CART-014; server blocks self-purchase `SELF_PURCHASE` "Cannot buy your own item"). DEV-TASK-66 (QA Task 10 FV4): "Request to Buy" now ALSO gated for the owner — on test-seller's own listing NEITHER Add to Cart NOR Request to Buy render (owner sees "You cannot view your own seller profile here" + bundle hint) |
...
| TRD-TC-M10 | Saved carts: max 3, server rejects 4th save, switch cart | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (3/3 cap; 4th save server-REJECTED "You already have 3 saved carts. Delete one to save a new one."; NO LRU eviction — all saved carts preserved; Switch works: saved→active + active→saved; guide updated to match). **Raw switch error FIXED + verified**: migration `20260830190000_fix_cart_switch_saved_limit_copy` (RPC pre-check → friendly structured `{code,message}`) + `CartScreen.confirmSwitchCart` safety net; RPC returns "You already have 3 saved carts. Delete one to save a new one." (no raw string). **QA Task 10 FV5 re-verify on-device**: built 3 saved + 1 active via UI — SAVE path "Could not save cart / You already have 3 saved carts…" AND SWITCH path "Could not switch / You already have 3 saved carts…" both friendly, no raw string |
...
| TRD-TC-M12 | Max SP available shown per cart item (subscriber) | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (subscriber-only "Accepts Points · Up to N SP" with REAL category cap: Bicycle 45 SP @Sports 75%, QA Canned 10 SP @Toys 50%, Puzzle 9 SP @Toys 50%; unavailable item reverts to plain "Accepts Points"). NOTE: test-buyer subscription was stale (current_period_end past → is_active_subscriber=false → numeric hidden); reversible fixture fix applied + reverted |
| TRD-TC-M13 | Realtime: item becomes unavailable while in cart | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (inline "This item is no longer available" within ~1s; subtotal EXCLUDES it: $98→$80 client == server `rpc_cart_validate_for_checkout` cart_total_cents 8000 + UNAVAILABLE_ITEMS; checkout blocked modal; badge reverts to plain "Accepts Points") |
...
| TRD-TC-M16 | Success toast appears and auto-dismisses on add-to-cart | 1 | AX-tree-exposure-verified, VoiceOver-interaction-not-directly-testable | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | Accepted R38 class: toast copy/color/non-blocking + 2500ms auto-dismiss + `announceForAccessibility` (SuccessToast:66) + `success-toast`/alert/liveRegion source-verified; 2.5s < tool round-trip → on-device visual/AX catch + utterance capture not directly testable (R38) |
...
| TRD-TC-M18 | Toast copy uses "Trade Basket" terminology | 1 | AX-tree-exposure-verified, VoiceOver-interaction-not-directly-testable | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | Copy "Added to Trade Basket" confirmed on ALL 3 add paths (direct / Save & Start New Cart / Replace Cart) + Trade Basket subtitles; same R38 scoping as M16 |
...
| TRD-TC-M20 | Discover header heart icon navigates to Favorites | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (DT-63 fixed drift: Heart icon + a11y label "View Favorites" (`discover-header-favorites`) → navigates to Favorites; navigation-only, no toggle). DEV-TASK-66 (QA Task 10 FV6): guide now correctly says neutral-gray heart (L2905 "neutral-gray heart icon … consistent with the header icon system"); on-device `discover-header-favorites` present — doc matches app |
