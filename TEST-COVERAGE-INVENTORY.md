...
| TRD-TC-M20 | Discover header heart icon navigates to Favorites | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (DT-63 fixed drift: Heart icon + a11y label "View Favorites" (`discover-header-favorites`) → navigates to Favorites; navigation-only, no toggle). Minor: icon renders neutral-gray (guide says pink/red) |
...
| TRD-TC-N02 | Admin minimum cart value validation | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (guide updated: no $5 floor, negative rejected at admin UI "Minimum cart value cannot be negative", ≥$0 saves, 0=no minimum). **NEW FINDING: RPC layer `upsert_admin_config_setting` accepts negative values (verified via `qa:admin-config-set` wrote −5) — negative guard is admin-UI-only, no server-side validation** |
