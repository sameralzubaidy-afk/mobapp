...
| TRD-TC-M05 | Cannot add own item to cart | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (own listing → Add to Cart hidden on-device, CART-014; server blocks self-purchase `SELF_PURCHASE` "Cannot buy your own item") — related UX: "Request to Buy" CTA still visible on own listing (reaches a server-blocked flow) |
...
| TRD-TC-M20 | Discover header heart icon navigates to Favorites | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (DT-63 fixed drift: Heart icon + a11y label "View Favorites" (`discover-header-favorites`) → navigates to Favorites; navigation-only, no toggle). DEV-TASK-66: guide updated — icon is neutral-gray (consistent with the header icon system) |
...
| TRD-TC-N02 | Admin minimum cart value validation | 1 | PASS | 2026-08-30 | `qa-task9-dt63-fix-verify-2026-08-30` | ✅ PASS (guide updated: no $5 floor, negative rejected at admin UI "Minimum cart value cannot be negative", ≥$0 saves, 0=no minimum). DEV-TASK-66: server-side guard added — `upsert_admin_config_setting` + `BEFORE UPDATE` trigger reject non-numeric/negative money keys |
