...
| **R3 — Delayed Payout + Buffer** | TC-R3-01 | Payout scheduled with buffer (release = completed_at + N) |
| | TC-R3-02 | Release at due date via cron + pending→available |
| | TC-R3-03 | Buffer 0 = immediate dispatch (regression) |
| | TC-R3-04 | Late completion → later release date off actual completed_at |
| | TC-R3-05 | Dispute holds buffered payout then releases |
| | TC-R3-06 | Admin buffer change applies to new completions |
| | TC-R3-07 | Manual withdrawal gated until release |
...
## Group Y — Tiered Buyer-Fee Engine (R1 — first-trade protection) (New Implementation)

> Added 2026-08-09. Covers the tiered buyer-fee engine: active members pay a flat
> "Safety & Platform Fee"; free users pay a flat fee on their first trade; free users
> with 1+ completed trades pay a percentage of the cash portion + a fixed fee, capped.
> All fee amounts are DYNAMIC from `admin_config` (fees category) — the values below
> are the seeded defaults and can be changed by an admin. First-trade eligibility is a
> STATE (`profiles.fee_state`) consumed ONLY on successful capture + completion.
>
> Config keys (admin_config, category `fees`): `buyer_fee_active_member_cents` (149),
> `buyer_fee_first_trade_cents` (149), `buyer_fee_subsequent_percentage` (5.0),
> `buyer_fee_subsequent_fixed_cents` (199), `buyer_fee_subsequent_max_cents` (499),
> `buyer_fee_label` ("Safety & Platform Fee").
>
> State machine: `no_completed_trade → first_trade_in_progress → first_trade_completed
> → subsequent_free → active_member` (stored on `profiles.fee_state`; `active_member`
> = subscription status `trial` or `active`).

### TC-Y01 · Active member (trial) — flat Safety & Platform Fee
**Objective:** Verify a buyer on a trial subscription pays the flat active-member fee.
**Precondition:** Buyer has subscription status = `trial`.
**Steps:**
1. As the trial buyer, open a cash-only item and proceed to the offer screen.
2. Note the fee line on the order summary (label from `buyer_fee_label`, amount from `buyer_fee_active_member_cents`).
3. Submit the offer.
**Expected Result:**
- The order summary shows one flat fee (default $1.49 / 149¢) with the configured label.
- `SELECT fee_state, completed_trade_count FROM profiles WHERE user_id='<buyer>'` → `active_member`.
- The trade row has `buyer_fee_state = 'active_member'` and `buyer_transaction_fee_cents = 149`.

### TC-Y02 · Active member (paid) — flat Safety & Platform Fee
**Objective:** Verify a paid subscriber pays the same flat fee.
**Precondition:** Buyer has subscription status = `active`.
**Steps:** Same as TC-Y01.
**Expected Result:** Same flat fee (default 149¢); `profiles.fee_state = 'active_member'`; trade snapshot `buyer_fee_state = 'active_member'`.

### TC-Y03 · Free user with no completed trade — flat fee on first trade
**Objective:** Verify first-trade protection: a free user with 0 completed trades pays the flat fee.
**Precondition:** Buyer free, `completed_trade_count = 0`, `fee_state = 'no_completed_trade'`.
**Steps:**
1. As the free buyer, submit an offer on any item.
2. Check the fee on the offer screen and after submission.
**Expected Result:**
- Fee shown = flat first-trade fee (default $1.49 / 149¢).
- After the offer is created: `profiles.fee_state = 'first_trade_in_progress'` (0 completed, active trade exists).
- Trade snapshot `buyer_fee_state = 'no_completed_trade'` or `'first_trade_in_progress'`; `buyer_transaction_fee_cents = 149`.

### TC-Y04 · Free user with 1+ completed trades — percentage fee (5% + $1.99, cap $4.99)
**Objective:** Verify a free user with one or more completed trades pays the percentage tier.
**Precondition:** Buyer free with ≥1 completed trade (`fee_state = 'first_trade_completed'` or `'subsequent_free'`).
**Steps:**
1. As the free buyer (1+ completed), open a $60.00 item (no SP).
2. Submit the offer.
**Expected Result:**
- Fee = min(199 + round(6000 × 5%), 499) = min(199 + 300, 499) = 499¢ ($4.99) — capped.
- For a $20.00 item: min(199 + 100, 499) = 299¢ ($2.99).
- Trade snapshot `buyer_fee_state` matches the percentage-tier state.

### TC-Y05 · Percentage applies ONLY to the cash portion (Swap Points excluded)
**Objective:** Verify the percentage fee is computed on cash after SP, never on the SP-covered amount.
**Precondition:** Free buyer with 1+ completed trades applies SP at checkout.
**Steps:**
1. As the free buyer (1+ completed), open a $60.00 item and apply $20 of SP (cash portion = $40.00).
2. Submit the offer.
**Expected Result:**
- Fee = min(199 + round(4000 × 5%), 499) = min(199 + 200, 499) = 399¢ ($3.99) — computed on the $40 cash portion, NOT the $60 price.
- SP never reduces the fee base to zero and never pays the fee (fee is cash-only).

### TC-Y06 · One fee per checkout regardless of bundle size (single seller, multi-item)
**Objective:** Verify a multi-item, single-seller checkout charges ONE fee (with `charge_one_fee_per_bundle` enabled).
**Precondition:** Admin has `charge_one_fee_per_bundle = true`; buyer in a flat-fee tier.
**Steps:**
1. Add 3 items from the SAME seller to the cart and check out.
**Expected Result:**
- The order summary shows ONE fee line (flat fee once, not ×3).
- Only the first item's trade carries `buyer_transaction_fee_cents`; items 2–3 carry 0.
- Total fee charged for the checkout = one flat fee.

### TC-Y07 · Cancelled / timed-out / failed-capture first trade does NOT consume first-trade eligibility
**Objective:** Verify eligibility is a state consumed only by completion — a cancelled/timed-out/failed first trade keeps the flat fee available for the next attempt.
**Precondition:** Free buyer, `fee_state = 'no_completed_trade'`.
**Steps:**
1. Submit a first-trade offer (state → `first_trade_in_progress`).
2. Cancel the trade (or let the offer expire / fail capture).
3. Submit a new offer on a different item.
**Expected Result:**
- After cancel/expiry/failed-capture: `profiles.completed_trade_count` stays 0; `fee_state` returns to `no_completed_trade` (no other active trade).
- The next offer still charges the flat first-trade fee (149¢) — eligibility NOT consumed.

### TC-Y08 · Full refund of the first completed trade restores eligibility
**Objective:** Verify a full refund of the trade that consumed first-trade eligibility restores the flat fee.
**Precondition:** Free buyer whose first trade completed (`completed_trade_count = 1`, `fee_state = 'first_trade_completed'`, trade `consumed_first_trade_eligibility = true`).
**Steps:**
1. Issue a FULL refund for that trade (price + fee + tax) via the refund flow / `rpc_record_payment_refund`.
**Expected Result:**
- `profiles.completed_trade_count` decrements to 0.
- `profiles.fee_state` reverts to `no_completed_trade` (or `first_trade_in_progress` if another active trade exists).
- The next offer charges the flat first-trade fee. A PARTIAL refund must NOT restore eligibility.

### TC-Y09 · Fee-state transitions are trigger-driven (no elapsed-time consumption)
**Objective:** Verify `profiles.fee_state` transitions only on trade/subscription events, never by time.
**Steps:**
1. Watch `profiles.fee_state` for a free buyer across: offer created → in_progress; completion → first_trade_completed; second completion → subsequent_free; subscription enters `trial`/`active` → active_member; subscription leaves `trial`/`active` → reverts to free state.
**Expected Result:** Every transition is caused by a `trades.status` change or a `subscriptions.status` change. No timer changes `fee_state` — so any future extension-consent wait (R15) requires no special fee handling.

### TC-Y10 · Order summary shows the correct fee + label (all tiers)
**Objective:** Verify the checkout/order-summary fee line reflects the resolved tier and label.
**Steps:**
1. For each tier (active member, first-trade free, 1+ completed free), open checkout and read the fee line.
**Expected Result:** The line shows the label from `buyer_fee_label` (e.g. "Safety & Platform Fee") and the amount computed by `fn_get_buyer_fee_for_checkout`. The displayed amount matches the trade's `buyer_transaction_fee_cents` after submission.

### TC-Y11 · Server-authoritative fee (client cannot understate the fee)
**Objective:** Verify the Edge Function recomputes the fee server-side even if the client sends a stale/wrong fee.
**Steps:**
1. Using a client that sends `transaction_fee_cents: 0` (or a stale value) with `cash_amount_cents` = cash portion only, submit an offer.
**Expected Result:**
- The stored `buyer_transaction_fee_cents`/`total_fee_cents` equals the server-resolved tiered fee (not the client value).
- The Stripe PaymentIntent amount includes the server fee; the buyer cannot bypass the fee by sending 0.
- Fee resolution failure returns a structured `CONFIG_UNAVAILABLE` error (no silent $0 fee).

---

## Group R3 — Delayed Seller Payout + Buffer (New Implementation)

> **Status:** Newly implemented 2026-08-10 (R3 — Delayed Seller Payout + Buffer, enforcement).
> These cases are **NOT yet executed**. They cover the payout buffer (`admin_config.payout_buffer_days`,
> default 2, admin-tunable 0–30) now being ENFORCED: payouts release on a stored `payout_release_at`
> computed from the ACTUAL completion timestamp (`trades.completed_at`) + the buffer value in effect at
> completion, funds sit in `seller_balance.pending_balance_cents` until release, and the `release-due-payouts`
> cron (→ Edge Function → `rpc_release_due_payouts`) dispatches due payouts. They are kept in their own
> section (not mixed with the tested groups) so what is already tested vs. newly implemented stays clear.
>
> Migration: `20260810000008_r3_delayed_payout_buffer.sql`. Functions: `release-due-payouts` (EF),
> `initiate-payout` (updated), `rpc_release_due_payouts`, `create_seller_payout_on_trade_completion`,
> `fn_queue_payout_on_complete`, `update_seller_balance_on_trade_completion`.
>
> Config key: `admin_config.payout_buffer_days` (0–30, 0 = release immediately) at `/settings/trade-timing` → Pickup & Payout.

### TC-R3-01 · Payout scheduled with buffer — release date = completed_at + buffer days

**Ref:** R3 (2026-08-10) · SYSTEM_REQUIREMENTS_V2 §1.6/§7.3 · TRADING-FLOW-V2 §6.3
**Actors:** test-buyer + test-seller
**Precondition:** Migration `20260810000008_r3_delayed_payout_buffer.sql` applied; `payout_buffer_days = 2` (default); seller has a verified payout method.

**Objective:** Verify that completing a cash trade schedules the payout, does NOT dispatch it immediately, and stores `payout_release_at = trades.completed_at + 2 days` on BOTH `seller_payouts` and `trades`.

**Steps:**
1. With `payout_buffer_days = 2`, complete a trade (buyer taps [I Got It] → [Confirm]).
2. Immediately check the payout row and trade:
```sql
SELECT sp.id, sp.status, sp.payout_release_at, sp.gross_amount_cents,
       t.status, t.payout_status, t.completed_at, t.payout_release_at
FROM seller_payouts sp JOIN trades t ON t.id = sp.trade_id
WHERE sp.trade_id = '<trade_id>';
```
3. Confirm the payout is `pending` (NOT `processing`) and `payout_release_at ≈ completed_at + 2 days`.
4. Open **SellerEarningsScreen** as the seller and confirm the pending payout shows **"Releases [date]"**.
5. Confirm `seller_balance.pending_balance_cents` increased (and `available_balance_cents` did not).

**Expected Result:**
- `sp.payout_release_at = t.completed_at + make_interval(days => 2)`; `sp.status = 'pending'`; `t.payout_status = 'pending'`.
- No `payout_sent` event / no `stripe_transfer_id` yet (payout NOT dispatched at completion).
- The seller sees a pending payout with the release date; buffered funds are pending, not available.

### TC-R3-02 · Release at due date — release-due-payouts dispatches and moves pending → available

**Ref:** R3 · `rpc_release_due_payouts` · `release-due-payouts` EF
**Actors:** test-admin (or cron) + test-seller
**Precondition:** A buffered pending payout exists (from TC-R3-01) with `payout_release_at` in the future.

**Objective:** Verify that once `payout_release_at` passes, the release processor dispatches the payout to `initiate-payout` and moves the funds from `pending_balance_cents` to `available_balance_cents`.

**Steps:**
1. Fast-forward the release date (`UPDATE trades SET payout_release_at = now() - interval '1 minute' WHERE id='<trade_id>';` — and mirror on `seller_payouts`).
2. Run `SELECT public.rpc_release_due_payouts(100);` — confirm it returns the trade in `trade_ids`.
3. Verify the trade moved to `processing`/`paid` (or `requires_action` if the provider dispatch fails) and `stripe_transfer_id` is set.
4. Confirm `seller_balance.available_balance_cents` increased and `pending_balance_cents` decreased by the same proceeds.
5. Re-run `rpc_release_due_payouts(100)` — confirm the trade is NOT re-selected (idempotent, no double payout).

**Expected Result:**
- The payout is dispatched exactly once after the release date; `seller_balance` moves pending → available once.
- Re-running the processor returns an empty `trade_ids` (or the same count without the released trade) — no double dispatch.

### TC-R3-03 · Buffer 0 = immediate dispatch (regression)

**Ref:** R3 (backward compat) · TRADING-FLOW-V2 §6.3.1
**Actors:** test-admin + test-buyer + test-seller
**Precondition:** Seller has a verified payout method.

**Objective:** Verify that with `payout_buffer_days = 0`, a completed trade dispatches immediately (the pre-R3 behavior) and `payout_release_at = completed_at`.

**Steps:**
1. In admin set **Payout Buffer = 0** at `/settings/trade-timing` → Pickup & Payout.
2. Complete a new trade and check the payout row within seconds.
3. Revert the buffer to the default (2) afterwards.

**Expected Result:**
- `payout_release_at = completed_at`; the trade's `payout_status` moves to `processing`/`paid` promptly (trigger fires immediately).
- Funds are credited to `available_balance_cents` at completion (not pending) — pre-R3 behavior preserved.

### TC-R3-04 · Late completion (extension) → later release date keyed off actual completed_at

**Ref:** R3 · "actual completion, not expected window"
**Actors:** test-buyer + test-seller
**Precondition:** `payout_buffer_days = 2`.

**Objective:** Verify the release date is computed from the ACTUAL completion timestamp, NOT the expected `auto_complete_at` window — so a trade completed late (e.g. via a granted extension) naturally gets a later release date with no special-case logic.

**Steps:**
1. Create an in-progress trade and advance past `auto_complete_at` WITHOUT completing (simulate an extension being granted).
2. Complete the trade late (buyer taps [I Got It] at T_late > `auto_complete_at`).
3. Check:
```sql
SELECT completed_at, auto_complete_at, payout_release_at,
       (payout_release_at = completed_at + make_interval(days => 2)) AS keyed_off_completed_at
FROM trades WHERE id='<trade_id>';
```

**Expected Result:**
- `keyed_off_completed_at = true` — `payout_release_at = completed_at + 2 days`, and `completed_at > auto_complete_at`.
- The release date is later than `auto_complete_at + 2 days` would have been; no special-case code exists for extensions.

### TC-R3-05 · Dispute holds a buffered payout; resolving Complete releases it via the cron

**Ref:** R3 · TRADING-FLOW-V2 §6.3.1 (dispute hold)
**Actors:** test-admin + test-buyer + test-seller
**Precondition:** A buffered pending payout exists; the trade can be disputed.

**Objective:** Verify a dispute raised at/after completion keeps the payout held (not dispatched), and resolving the dispute as Complete makes it eligible for the release cron.

**Steps:**
1. Open a dispute on the completed trade (or complete a trade while a dispute is reported). Confirm `payout_status` stays `pending` and `rpc_release_due_payouts` does NOT select it.
2. Admin resolves the dispute as **Complete** (`dispute_resolution = 'resolved_seller'`, `dispute_status = 'resolved'`).
3. Fast-forward the release date if needed and run `SELECT public.rpc_release_due_payouts(100);`.

**Expected Result:**
- While `dispute_status IN ('reported','under_review')`, the payout is held and the release processor skips it.
- After resolution to Complete, the release processor dispatches it (single payout, idempotent).

### TC-R3-06 · Admin buffer change applies to NEW completions with no code deploy

**Ref:** R3 · N1 configurability (BRD §6.8)
**Actors:** test-admin + test-buyer + test-seller
**Precondition:** `payout_buffer_days` currently 2.

**Objective:** Verify an admin change to `payout_buffer_days` takes effect on the NEXT trade completion without a deploy (existing pending payouts are unaffected).

**Steps:**
1. In admin set **Payout Buffer = 5**; Save.
2. Complete a new trade; confirm `payout_release_at = completed_at + 5 days`.
3. Revert to 2 (or the desired value) afterwards. Confirm an EARLIER buffered payout's release date did NOT change.

**Expected Result:**
- New completions read the live `payout_buffer_days` (via `fn_admin_config_int`) and store the corresponding release date; existing payouts keep their stored release date.
- `SELECT public.fn_admin_config_int('payout_buffer_days', 2);` returns the new value.

### TC-R3-07 · Manual withdrawal gated until the release date

**Ref:** R3 · `requestPayoutWithdrawal` (mobile) + `request_seller_payout` (RPC) + `seller_balance`
**Actors:** test-seller (manual/auto mode)
**Precondition:** A buffered pending payout exists (release date in the future).

**Objective:** Verify the seller cannot withdraw/request a payout before the release date, and CAN once it passes.

**Steps:**
1. As the seller, open the payout/withdrawal flow for the buffered payout before the release date.
2. Attempt to withdraw (Request Payout / Request Withdrawal).
3. After the release date passes (and the release processor runs), retry.

**Expected Result:**
- Before release: the withdrawal is blocked with a clear message ("releases on [date]") or the amount is not in `available_balance_cents`; `request_seller_payout` returns an insufficient/locked-funds error.
- After release: the funds are in `available_balance_cents` and the withdrawal succeeds — no double payout (the trade payout was released exactly once).

---

## Group Y2 — Tiered Buyer-Fee on Subscription Marketing Screens (R1) (New Implementation)

> Added 2026-08-09. The 4 subscription marketing/plan screens
> (`PlanComparisonScreen`, `KidsClubOverviewScreen`, `ManageKidsClubScreen`,
> `SubscriptionPaymentScreen`) now present the R1 tiered buyer fee: members pay a
> flat **Safety & Platform Fee** (dynamic from `buyer_fee_active_member_cents`),
> and the free tier is described honestly as "flat on 1st trade, then %". No stale
> "$0.99 vs $2.99" copy remains anywhere in the subscription flow. All values are
> dynamic from admin_config and picked up on the next screen load.

### TC-Y2-01 · Plan Comparison shows the tiered fee honestly
**Objective:** Verify the plan-comparison table no longer shows the legacy flat $0.99/$2.99.
**Steps:**
1. Open the subscription **Plan Comparison** screen.
2. Read the "Transaction fee (per trade)" row.
**Expected Result:**
- **Free** column reads "Flat on 1st trade, then %" (no dollar figure that implies a single flat free fee).
- **Kids Club+** column reads the flat member fee from `buyer_fee_active_member_cents` (default "$1.49 flat"), dynamic.

### TC-Y2-02 · Kids Club Overview shows the flat member fee
**Objective:** Verify the Kids Club+ overview benefits list uses the tiered flat fee.
**Steps:**
1. Open the **Kids Club+** overview screen.
2. Read the fee benefit item.
**Expected Result:** It reads "Pay a flat $1.49 Safety & Platform Fee on every trade" (amount from `buyer_fee_active_member_cents`, label from `buyer_fee_label`). No "vs $2.99" wording.

### TC-Y2-03 · Manage Kids Club shows the flat member fee benefit
**Objective:** Verify the Manage subscription screen benefits reminder uses the tiered fee.
**Steps:**
1. As an active/trial subscriber, open **Manage Kids Club**.
2. Read the "Kids Club+ Benefits" list.
**Expected Result:** The fee line reads "✓ Flat $1.49 Safety & Platform Fee on every trade" (dynamic amount). No "(vs $2.99)".

### TC-Y2-04 · Subscription Payment shows the flat member fee
**Objective:** Verify the payment screen benefit uses the tiered fee.
**Steps:**
1. Open **Subscription → Payment** (join/renew).
2. Read the "Lower Transaction Fees" benefit.
**Expected Result:** It reads "Pay a flat $1.49 Safety & Platform Fee on every trade" (dynamic). No "for free users" dollar comparison.

### TC-Y2-05 · Admin changes propagate to all subscription screens
**Objective:** Verify a fee-config change is reflected on the marketing screens on the next load (no cache staleness).
**Steps:**
1. As admin, change `buyer_fee_active_member_cents` on **Settings → Trade Timing** (e.g., to 199).
2. Open each of the 4 subscription screens (Plan Comparison, Kids Club Overview, Manage, Payment).
**Expected Result:** Every screen shows the new flat fee ($1.99) with the configured label. A pull-to-refresh / remount picks up the change (each screen calls `getActiveMemberFeeCents(true)` which bypasses the 5-min config cache).
3. Restore the value to 149.

### TC-Y2-06 · Regression — no legacy "$0.99 / $2.99" fee copy remains
**Objective:** Confirm the old flat-fee comparison is fully gone from the subscription flow.
**Steps:**
1. Grep the subscription screens for legacy fee strings / functions:
```bash
grep -rn "transaction_fee_subscriber_cents\|transaction_fee_non_subscriber_cents\|0.99 per transaction\|2.99 per transaction\|vs .*for free users" p2p-kids-marketplace/src/screens/subscription
```
**Expected Result:** No matches in the 4 marketing/plan screens. (The legacy `getTransactionFeeSubscriberCents`/`NonSubscriberCents` functions still exist in `adminConfig.ts` for backward compat, but no screen renders "$0.99 vs $2.99".)

---

## Group Z — R11 Category SP Caps & R6 SP Entitlement (New Implementation)

> Added 2026-08-09. R11 makes the per-category SP config (spend-cap %, multiplier m,
> optional absolute cap) ENFORCED server-side by R5 checkout (Edge Function + DB
> trigger guard). R6 gates SP earn/spend by subscription state: trial/paid earn+spend;
> grace spends existing SP (no new earn, pays free fee); post-grace frozen (not
> deleted); resubscribe restores.

### TC-Z01 · Admin sets category spend-cap % + multiplier → R5 enforces at checkout
**Actors:** test-admin + test-buyer (subscriber) + test-seller
**Objective:** Verify an admin-set category cap % and multiplier are enforced end-to-end at checkout.
**Steps:**
1. As admin, on **Categories → edit a category**, set **SP Spending Cap %** = 60 and **SP Earning Multiplier** = 1.30. Save.
2. test-seller lists a $50 item in that category (Accept SP).
3. test-buyer opens the offer screen — the SP slider max = 30 SP (60% of $50).
4. Submit an offer using 25 SP; complete the trade.
**Expected Result:** The slider clamps at 30 SP. On completion the seller receives the buyer's 25 SP plus the platform bonus (net-new (m−1)×S); the buyer's SP is reserved/spent correctly. No SP above the cap can be submitted from the app.

### TC-Z02 · Category spend-cap % overrides the global 50%
**Actors:** test-admin + test-buyer (subscriber)
**Objective:** Verify a category configured above 50% allows more than half the price in SP (per owner decision).
**Steps:**
1. As admin, set a category's **SP Spending Cap %** to 70 on a $30 item.
2. test-buyer opens the offer screen.
**Expected Result:** The SP slider max = 21 SP (70% of $30), not 15. The buyer can apply up to 21 SP; the remainder + platform fee are paid in cash.

### TC-Z03 · Admin sets absolute SP Redemption Cap → enforced per item
**Actors:** test-admin + test-buyer (subscriber)
**Objective:** Verify the optional absolute per-item SP cap (`sp_redemption_cap`) is enforced even when the % cap is higher.
**Steps:**
1. As admin, set a category's **SP Redemption Cap** to 10 (absolute) on a $50 item (spend-cap % 60 → 30 SP by %).
2. test-buyer opens the offer screen.
**Expected Result:** The SP slider max = 10 SP (absolute cap binds below the % cap).

### TC-Z04 · Server rejects over-cap SP from a stale/edited client (EF + DB guard)
**Actors:** test-buyer (subscriber)
**Objective:** Verify the server rejects an SP amount above the cap even if the client is edited/stale — not just the UI clamp.
**Steps:**
1. Open an item whose category cap is 15 SP (50% of $30).
2. Submit an offer with `sp_amount` = 20 via a dev tool / modified request (bypass the UI).
**Expected Result:** The offer is rejected with `SP_CAP_EXCEEDED` (Edge Function) and the DB reserve trigger also raises — no trade row is created, no SP is reserved, no Stripe hold is created.

### TC-Z05 · Bundle checkout: over-cap item fails cleanly, others succeed
**Actors:** test-buyer (subscriber)
**Objective:** Verify a bundle where one item's SP exceeds its cap fails that item but the other items still process.
**Steps:**
1. Add two same-seller items to the cart; item A cap = 10 SP, item B cap = 30 SP.
2. Apply 20 SP to item A (over its cap).
**Expected Result:** Item A is rejected (over cap) with a clear error; item B (and any valid item) proceeds. No partial SP reservation on the failed item.

### TC-Z06 · R6: Grace-period buyer can spend existing SP at checkout
**Actors:** test-buyer (in grace period) + test-seller
**Objective:** Verify a grace-period subscriber can still redeem existing SP.
**Steps:**
1. Put test-buyer in `grace_period` status with wallet state `grace_period` and a positive available SP balance.
2. Open an Accept SP item and submit an offer using SP.
**Expected Result:** The offer submits successfully — SP is reserved. `get_subscription_summary` returns `can_spend_sp = true`; the SP wallet banner reads "you can still spend existing SP".

### TC-Z07 · R6: Grace-period seller does NOT earn new SP (platform bonus gated)
**Actors:** test-seller (in grace period) + test-buyer
**Objective:** Verify a grace-period seller receives the buyer's SP but no newly-created platform bonus.
**Steps:**
1. Put test-seller in `grace_period`; complete a trade that used buyer SP on an Accept SP listing.
**Expected Result:** The seller receives the buyer's transferred SP in pending, but `seller_sp_bonus` = 0 (no net-new SP). An active-subscriber seller in the same scenario DOES receive the bonus.

### TC-Z08 · R6: Post-grace (expired) SP is FROZEN — cannot spend
**Actors:** test-buyer (expired)
**Objective:** Verify that after the grace period ends the wallet is frozen (not deleted) and SP cannot be spent.
**Steps:**
1. Move test-buyer to `expired` status; run the grace-period cron (or `rpc_set_sp_wallet_state(user,'frozen')`).
2. Open an Accept SP item and attempt to use SP.
**Expected Result:** The wallet state is `frozen`; the SP balance is still visible (not deleted); any SP use is blocked (`SP_NOT_ENTITLED` / "wallet is frozen").

### TC-Z09 · R6: Resubscribe restores frozen SP balance
**Actors:** test-buyer (expired → resubscribed)
**Objective:** Verify resubscribing makes the previously-frozen SP spendable again.
**Steps:**
1. With test-buyer's wallet `frozen` and a balance, renew the subscription (trial conversion / payment succeeds).
2. Check the wallet state and attempt to spend SP.
**Expected Result:** The wallet state returns to `active` (via `rpc_set_sp_wallet_state`), `frozen_at` is cleared, and the full balance is spendable again. No SP was lost.

### TC-Z10 · R6: Free user cannot spend SP (entitlement blocks)
**Actors:** test-free
**Objective:** Verify a free (never-subscribed) user cannot redeem SP even with a wallet.
**Steps:**
1. Ensure test-free has a non-zero available SP balance (or create one).
2. Open an Accept SP item and attempt to use SP.
**Expected Result:** SP use is blocked (`SP_NOT_ENTITLED`); the UI shows the Kids Club+ upsell. No reservation occurs.

### TC-Z11 · R13: Grace-period buyer pays the free-user fee at checkout
**Actors:** test-buyer (grace period)
**Objective:** Verify a grace-period buyer is charged the free-user (non-member) fee.
**Steps:**
1. With test-buyer in `grace_period` (1+ completed trades), open checkout on a cash item.
**Expected Result:** The buyer fee is the free-user tier (5% of cash portion + $1.99, capped at $4.99), NOT the member flat fee. `fn_get_buyer_fee_for_checkout` treats grace as a non-member.

### TC-Z12 · R5 regression: seller receives m×S with 3-day pending window unchanged
**Actors:** test-seller (subscriber) + test-buyer (subscriber)
**Objective:** Verify the multiplier earn + 3-day pending release still work after R11/R6 changes.
**Steps:**
1. Complete an SP trade on a category with multiplier 1.30.
**Expected Result:** Seller pending balance = S + FLOOR(S × 0.30); `pending_sp_release_at` = completion + 3 days; `rpc_release_pending_sp` moves it to available after the window.

---

## Group Y — R8 Core Trust & Safety Gates + N4 18+ Registration Gate

> Added 2026-08-09. R8: listings must pass Google Vision image moderation before admin approval (approval-gate). N4: no user under 18 may register (hard gate; nothing persisted). See `docx/SYSTEM_REQUIREMENTS_V2.md` §10.2 / §14.1.

### TC-R8-01 · New listing is hidden (pending) until admin approval
**Actors:** test-seller + any buyer
**Objective:** Verify a newly created listing is not publicly visible until approved.
**Steps:**
1. Log in as test-seller and publish a listing (with images).
2. As a buyer in the same node, search/discover for the item.
3. As test-seller, open **My Items**.
**Expected Result:** The listing is created with status `pending`; it does NOT appear in discovery/search for buyers; the seller sees the pending-review message; it only appears publicly after admin approval.

### TC-R8-02 · Admin cannot approve until AI image moderation completes
**Actors:** test-admin + test-seller
**Objective:** Verify the approval gate blocks while images are unreviewed.
**Steps:**
1. Create a listing with images but no moderation decision yet (or pause the `moderate-image` run).
2. Admin opens the pending listing in the admin portal and taps **Approve Listing**.
**Expected Result:** Approval is rejected with `MODERATION_IN_PROGRESS` ("AI moderation is still reviewing its images"); the detail panel shows "⏳ Reviewing images". After moderation completes (all images `approved`), approval succeeds.

### TC-R8-03 · Flagged image blocks approval (must reject or replace)
**Actors:** test-admin + test-seller
**Objective:** Verify a listing with a flagged image cannot be approved.
**Steps:**
1. Upload a listing image that Google Vision flags (LIKELY/VERY_LIKELY adult/violence content).
2. Admin opens the listing and taps **Approve Listing**.
**Expected Result:** Approval is rejected with `MODERATION_BLOCKED_FLAGGED`; the item status becomes `flagged`; the detail panel shows "⛔ Flagged"; admin must Reject or Request Edits instead.

### TC-R8-04 · Moderation disabled by admin config → approval allowed
**Actors:** test-admin
**Objective:** Verify the gate is bypassed only when AI moderation is off.
**Steps:**
1. Admin sets `moderation_ai_enabled` = false in admin config.
2. Approve a pending listing whose images have no moderation decision.
**Expected Result:** Approval succeeds; detail panel shows "🚫 Disabled by admin config". (Re-enable `moderation_ai_enabled` = true afterwards.)

### TC-R8-05 · CPSC recall match → listing flagged for admin review
**Actors:** test-seller + test-admin
**Objective:** Verify CPSC recall flagging (SAFETY-002) still surfaces in the admin queue.
**Steps:**
1. Create a listing whose title/description matches a known recall (e.g., "drop-side crib").
2. Admin opens **Items → Flagged** (or Listings search filtered to `flagged`).
**Expected Result:** An `item_safety_flags` row exists (flag_type `cpsc_recall`); the item appears in the admin flagged queue with the recall product/reason; it is not publicly visible.

### TC-R8-06 · Phone verification required to publish
**Actors:** test-buyer (unverified phone)
**Objective:** Verify the AUTH-V3-008 phone-verification gate blocks listing creation.
**Steps:**
1. Log in as a user with an unverified phone; attempt to publish a listing.
**Expected Result:** A phone-verification modal appears before publish; the listing cannot be created until the phone is verified.

### TC-R8-07 · Seller ID verification is optional (not required to sell)
**Actors:** test-seller
**Objective:** Verify ID badge is an optional trust badge.
**Steps:**
1. As a seller, list an item WITHOUT completing ID verification.
**Expected Result:** The listing can still be submitted and approved (no ID requirement to sell); ID verification remains available as an optional badge.

### TC-N4-01 · Under-18 registration is blocked (client gate)
**Actors:** prospective user
**Objective:** Verify the signup DOB gate refuses under-18 before any account is created.
**Steps:**
1. Open the live Signup screen and enter a DOB implying age < 18 (e.g., 2010-01-01).
2. Attempt to continue.
**Expected Result:** The form blocks with "Sorry, you must be 18 years old to register."; no `auth.users`/`profiles`/`subscriptions` row is created (verify via SQL that the email does not exist).

### TC-N4-02 · Under-18 registration is blocked server-side (backstop)
**Actors:** QA (SQL)
**Objective:** Verify the `on_auth_user_min_age` trigger aborts signup for under-18.
**Steps:**
1. Attempt `auth.admin.createUser` (or signUp) with `user_metadata.dob` = '2010-01-01'.
**Expected Result:** The insert fails with `AGE_MINIMUM_REQUIRED`; no auth user is persisted; a `debug_logs` row is written with `BLOCKED_UNDER_MIN_AGE`.

### TC-N4-03 · 18+ registration succeeds
**Actors:** prospective user
**Objective:** Verify an 18+ signup completes normally.
**Steps:**
1. Enter a DOB implying age >= 18 (e.g., 1990-01-01) and complete signup.
**Expected Result:** Account is created; the user proceeds to phone verification; `is_signup_age_allowed('1990-01-01')` returns TRUE.

### TC-N4-04 · No personal data collected from blocked under-18 users
**Actors:** QA (SQL)
**Objective:** Verify blocked under-18 attempts leave no data.
**Steps:**
1. Attempt several under-18 signups (client + server).
2. Query `auth.users` and `profiles` for those emails.
**Expected Result:** Zero rows exist for the blocked emails (client gate prevents the call; server trigger aborts the insert); only `debug_logs` audit rows (no PII beyond email) record the attempt.
...
| Payout scheduled with buffer — release = completed_at + N (R3) | TC-R3-01 |
| Payout releases at due date via cron + pending→available (R3) | TC-R3-02 |
| Buffer 0 = immediate dispatch (R3 regression) | TC-R3-03 |
| Late completion → later release date off actual completed_at (R3) | TC-R3-04 |
| Dispute holds buffered payout; resolves → cron release (R3) | TC-R3-05 |
| Admin buffer change applies to new completions (R3) | TC-R3-06 |
| Manual withdrawal gated until release (R3) | TC-R3-07 |
